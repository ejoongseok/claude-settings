...
> **외부-가시 문서** (고객사·외부 참석자 미팅 한정) — [`rules/external-doc.md`](../../rules/external-doc.md) 10원칙 준수. 팀 내부 비공식 회의는 유연 적용. 외부 회의는 내부 경로 금지, What > How, 정량·절대 표기, 구두 약속 원 표현 보존.
...
|--------|------|:------:|------------|
| 프로젝트 전체 구조 (모듈·도메인 약어) | `CLAUDE.md` | 선택 | 약어 풀네임 구체화 생략 — 원문 표현 그대로 기록 |
| 아키텍처 온보딩 (여정↔모듈, 이벤트 흐름) | `bot/INDEX.md` 또는 `.local.claude/ONBOARDING.md` | 선택 | 모듈 매핑 생략 — 회의 내용만으로 정리 |
...
**참조 전략**:
- 회의 내용에 도메인 용어나 프로젝트 모듈명이 등장하면 → `CLAUDE.md` 의 도메인 약어 참조하여 정확한 명칭 반영
  - 예: 약어 등장 시 → 풀네임으로 구체화
  - 예: "{모듈} 성능 이슈" → 해당 모듈의 실제 컴포넌트·기술 맥락 반영 (modules/{name}.md 참조)
- 고객 요구사항이 어느 모듈에 해당하는지 판단할 때 → 관련 `modules/{name}.md` 참조
...
> **[메타인지]** 갭 분석 전 자기 검증:
> 1. 근거 재점검 — 결정 사항·구두 약속·액션 아이템이 입력에 실제 있는가? 추측·보완이 섞이지 않았는가?
> 2. 전제 검증 — 회의록이 유효하려면 어떤 전제(참석자 식별, 모듈 맥락, 고객사 상황)가 맞아야 하는가?
> 3. 반대 증거 — "미참석자가 읽었을 때 '이 결정 왜?' 또는 '이 약속 구체 범위?'에 답할 수 없는 부분은?" 반박 1개 이상 → 갭 질문 우선순위에 반영
...
Frontmatter (CONTRACT §7-2 표준): `category: meetings, retention: 30d, harvest_targets: [biz-rules.md, customers/*.md]`

회의록이 확정되면 반드시 마크다운 파일로 저장한다.

### 프로젝트 연관 판단
1. `$ARGUMENTS`에 프로젝트명이 있으면 해당 프로젝트
2. 대화 맥락에서 프로젝트가 특정되면 해당 프로젝트 (예: "{프로젝트 키워드} 회의" → `.local.claude/projects/{프로젝트명}`)
3. `ls .local.claude/projects/`로 기존 프로젝트 확인 후 관련 있으면 해당 프로젝트
4. 어디에도 해당 안 되면 독립 저장

### 저장 경로
- 프로젝트 관련 → `.local.claude/projects/{project-name}/meetings/YYYY-MM-DD-{간략설명}.md`
- 독립적 → `.local.claude/meetings/YYYY-MM-DD-{간략설명}.md`
...
| 질문 | 담당 | 이 스킬에서 |
|------|------|-----------|
| 회의 메모·전사본을 구조화된 회의록으로 정리 (갭 질문 반복) | **이 스킬** | 핵심 — 회의록 작성 SSOT |
| 최근 커밋 기반 오늘의 변경·영향 브리핑 | /briefing | 다루지 않음 — 코드 변경 분석은 위임 |
| 회의록 내용을 수신자에 맞는 메시지/보고로 가공 | /draft | 회의록 경로 전달만 — 전달 가공은 위임 |

## 제약조건

- **외부-가시 문서 공통 원칙 준수** — `rules/external-doc.md` (가독성 5 + 정직성 5). 고객사 미팅·외부 참석자 전제 회의록은 10원칙 공통 적용. 팀 내부 비공식 회의록은 유연 적용 가능.
...
- 액션 아이템은 반드시 "검증 가능한 형태"로. "검토한다" (X) → "검토하여 [산출물]을 [기한]까지 공유한다" (O)
...
→ 1단계: 팀 내부 회의 판단
→ 2단계: 팀 내부 회의 구조로 1차 정리 (`CLAUDE.md` 에서 모듈·기술 스택 확인)
