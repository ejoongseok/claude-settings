...
> **외부-가시 문서**: [`rules/external-doc.md`](../../rules/external-doc.md) 10원칙 준수. 전달 초안은 본질적으로 외부 독자 대상. 내부 경로와 상수명 금지, Outside-in 서술, 스캔 가능성 우선(표 > 불릿 > 산문).
...
#### 3-1. 도메인 정확성
- 상태 전이 설명이 `biz-rules.md` 규칙에 맞는지 (예: "결재 완료 후 수정 가능"이라고 썼지만 실제로는 불가)
...
> 2. 전제 검증: 초안이 유효하려면 수신자의 어떤 사전 지식과 맥락이 필요한가? 그 전제가 실제로 성립하는가?
...
### 저장 경로
- 프로젝트 관련이면 `.local.claude/projects/{project-name}/draft/YYYY-MM-DD-{제목요약}.md`
- 독립적이면 `.local.claude/draft/YYYY-MM-DD-{제목요약}.md`
