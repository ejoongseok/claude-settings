...
| 질문 | 담당 | 이 스킬에서 |
|------|------|-----------|
| 들어온 요청을 해석·분류 (나가는 게 아니라 받는 것) | `/analyze-request` | [FAIL] 다루지 않음 (산출물을 입력으로는 받음) |
| 고객 문의·CS 접수 내용 분석 | `/cs` | [FAIL] 다루지 않음 (산출물을 전달용으로는 가공) |
| 변경 코드의 결함·품질 리뷰 | `/review` | [FAIL] 다루지 않음 (리뷰 결과를 전달용으로는 가공) |
| 나가는 메시지·보고를 수신자에 맞게 가공 | **이 스킬** | [OK] 핵심 |
...
- 전달 산출물이 길어지면 원본 문서는 참고 링크로 두고 발췌만 본문에 — 전체 복붙 금지.
