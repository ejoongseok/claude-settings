...
> **외부-가시 문서**: [`rules/external-doc.md`](../../rules/external-doc.md) 10원칙(가독성 5 + 정직성 5) 준수. PR은 동료, 리뷰어, 후임자 참조 전제. 내부 경로(`.claude/*`, `.local.claude/*`) 금지, 내부 상수와 플래그명 금지, 추측/확정 구분.
...
| 질문 | 담당 | 이 스킬에서 |
|------|------|-----------|
| 작업 항목 트래킹과 할 일 등록 | `/issue` | [FAIL] 다루지 않음 |
| 작성 중인 변경을 미리 점검 | `/draft` | [FAIL] 다루지 않음 |
| 변경 코드의 결함과 품질 리뷰 | `/review` | [FAIL] 다루지 않음 |
| 완료된 브랜치 변경을 PR 제목과 본문으로 작성 | **이 스킬** | [OK] 핵심 |

## 외부 데이터 의존

| 데이터 | 경로 | 필수/선택 | 부재 시 동작 |
|--------|------|:------:|------------|
| PR 템플릿 | `.github/PULL_REQUEST_TEMPLATE.md` | 선택 | CONTRACT 기본 구조(요약/변경사항/테스트/체크리스트)로 생성 |
| 프로젝트 컨텍스트 | `CLAUDE.md` | 선택 | base 브랜치와 컨벤션 일반 가정으로 진행, `[프로젝트 규칙 미확인]` 태그 |
| 팀원 매핑 | `.local.claude/team.md` | 선택 | git author 그대로 표기, 리뷰어 자동 추천 생략 |
| 커밋과 diff 이력 | `git log`, `git diff` (현재 브랜치) | 필수 | 변경 내용 추출 불가. "커밋 후 재실행" 안내 후 중단 |
...
5. **이슈 번호**: 브랜치명에서 추출 (`Feature/#123` 에서 `#123` 추출)
...
### 톤 & 수준
- **받는 쪽이 못 여는 참조 금지**: `.local.claude/...`, `STATUS.md §10.4`, `ANALYSIS.md` 같은 내부 전용 문서 경로를 PR 본문에 넣지 않음. GitHub PR은 팀 외부에도 보일 수 있고, 내부 문서 경로는 의미 없는 소음. 기준은 받는 사람이 열어볼 수 있는가이며, 같은 리포지토리의 코드 경로와 줄번호, PR과 이슈 번호, 공개 URL은 참조해도 됨
...
- git diff에서 변경된 파일 유형을 분석하여 자동 체크
  - `*Ctrl.java`, `*Svc.java` 변경이면 API 변경
  - `*.sql`, DDL 파일이면 DB 스키마 변경
  - `application*.yml` 이면 설정 파일 변경
  - 기존 메서드 시그니처 변경이면 기존 기능 영향 있음
...
- **풀스택** (FE + BE): 프론트 페이지에서 시작해 컨트롤러, 서비스, 데이터 접근 순
- **백엔드만**: 컨트롤러에서 시작해 서비스, 데이터 접근 순
- **프론트만**: 프론트 페이지에서 시작해 AJAX 호출, API 엔드포인트 순
...
| 조건 | 다이어그램 유형 | 예시 |
|------|---------------|------|
| 모듈 간 이벤트 흐름 변경 | flowchart | {모듈A} 에서 {모듈B} 로 Event 전달 |
| 상태 전이 로직 변경 | stateDiagram-v2 | {상태1} 에서 {상태2} 를 거쳐 {상태3} 로 전이 |
| DB 테이블 관계 변경 | erDiagram | {부모} \|\|--o{ {자식} |

- 조건에 해당하지 않으면 생략 (무조건 넣지 않는다)
- 노드 5개 이하로 간결하게. 복잡하면 핵심 흐름만 추출
- 기존 흐름과 변경 흐름 비교가 필요하면 Before/After로 나눠서 표현
...
Frontmatter (CONTRACT 7-2절 표준): `category: prs, retention: 30d`
...
> **[메타인지]** 출력 전 자기 검증 (external-doc 10원칙 준수):
> 1. 근거 재점검: 주요 변경사항이 실제 diff 기반인가? 추측과 과장 없는가?
...
- PR과 함께 추적할 작업 항목이 필요하면 `/issue`
- 변경 코드의 결함과 품질을 먼저 점검하려면 `/review`
- 변경 내용을 수신자별 메시지로 공유하려면 `/draft`
- 커밋 이력 부재 시 커밋 후 재실행

## 제약조건

- **외부-가시 문서 공통 원칙 준수**: `rules/external-doc.md` (가독성 5 + 정직성 5). PR은 동료, 리뷰어, 후임자가 참조. 원칙 재진술은 해당 문서 참조.
- 출력만 한다. 사용자가 명시적으로 요청하지 않으면 PR 생성과 push 금지.
- 주요 변경 사항은 실제 diff 기반. 추측과 과장 금지, 불확실하면 추측/확정 구분 표기.
- 내부 경로(`.claude/*`, `.local.claude/*`), 내부 상수, 플래그명을 PR 본문에 노출 금지.
- PR 본문은 자기 완결적. 외부 문서로 떠넘기지 않고 변경 의도, 테스트 방법, 영향을 본문에 직접 서술.
