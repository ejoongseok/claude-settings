...
| 질문 | 담당 | 이 스킬에서 |
|------|------|-----------|
| 작업 항목 트래킹·할 일 등록 | `/issue` | [FAIL] 다루지 않음 |
| 작성 중인 변경을 미리 점검 | `/draft` | [FAIL] 다루지 않음 |
| 변경 코드의 결함·품질 리뷰 | `/review` | [FAIL] 다루지 않음 |
| 완료된 브랜치 변경을 PR 제목·본문으로 작성 | **이 스킬** | [OK] 핵심 |
...
| 프로젝트 컨텍스트 | `CLAUDE.md` | 선택 | base 브랜치·컨벤션 일반 가정으로 진행, `[프로젝트 규칙 미확인]` 태그 |
| 팀원 매핑 | `.local.claude/team.md` | 선택 | git author 그대로 표기, 리뷰어 자동 추천 생략 |
| 커밋·diff 이력 | `git log`, `git diff` (현재 브랜치) | 필수 | 변경 내용 추출 불가 — "커밋 후 재실행" 안내 후 중단 |
...
- 출력만 한다. 사용자가 명시적으로 요청하지 않으면 PR 생성·push 금지.
- 주요 변경 사항은 실제 diff 기반. 추측·과장 금지, 불확실하면 추측/확정 구분 표기.
