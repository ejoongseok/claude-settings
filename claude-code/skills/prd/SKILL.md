...
> **외부-가시 문서** — [`rules/external-doc.md`](../../rules/external-doc.md) 10원칙 준수. PRD는 기획·경영진·고객 담당자·개발팀 공통 참조 문서. 내부 경로 금지, Outside-in(사용자 가치 → 구현), What > How, 정량·절대 표기.
...
| 아키텍처 온보딩 | `bot/INDEX.md` 또는 `.local.claude/ONBOARDING.md` | 핵심 사용자 여정↔모듈 매핑, 이벤트 흐름도 |
...
- **사례가 있나?** — 유사한 시도나 기술을 적용한 사례를 수집했는가? 검증 없이 도입하면 리스크.
  - 예: "욕설 필터링 유료 서비스 도입" 전에 → 현재 욕설이 얼마나 쓰이는지 먼저 파악 → 빈도가 낮으면 도입 가치 없음
...
①에서 "근본이 다른 곳"이면 → 근본 문제를 먼저 제시. ②에서 빈도/효용이 불분명하면 → 측정/로깅을 먼저 제안. ③에서 더 단순한 방법이 있으면 → "이런 방법도 있는데요"를 먼저 제시. 그래도 PRD가 필요하면 진행.
...
7. 의존성/리스크 미파악 — 다른 팀·시스템 의존 시 일정 위험
...
- 이게 안 되면 **지금은 어떻게** 하고 있나? (현재 워크어라운드 → 진짜 Pain Point 드러남)
