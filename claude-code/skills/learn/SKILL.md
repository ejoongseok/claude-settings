...
> 1. 근거 재점검 (팩트체크 판정 `[OK]`/`[FAIL]`/`[WARN]` 이 실제 코드나 문서 인용에서 도출되는가 vs 추측인가)
> 2. 전제 검증 (해당 문서에 이미 같은 내용이 다른 표현으로 있지 않은가, 150줄 상한과 중복 방지가 적용됐는가)
...
- 대응: `[코드 외 지식]` 태그 부착 후 저장. "이 지식은 현재 코드와 불일치, 개념이나 외부 원리로만 보관"
