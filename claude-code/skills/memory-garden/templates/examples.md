...
**동작**: Phase 6-1 아카이브, `{auto memory}/_archive/2026-04/` 로 이동
...
- 후속 처리 확인됨이면 **Resolved**
- 후속 처리 불명이면 **Stale**(사용자 확인) 또는 **Outdated**(갱신 필요)
...
**Phase 3 판정**:
- A는 **Active**, confidence High (전체 프로젝트 라이브)
- B는 **Duplicate** 후보, confidence Med
...
**본문 요지**:
> project-gamma TF는 PL 1명 단독 담당. 2026-04-16 추가로 백엔드 개발자 1명 공식 배정, 프론트 개발자 1명 빠짐.
...
**원인**: C2가 추상 서술. 근거 부족으로 Phase 4-2 강등 규칙 적용
...
- 같은 본문 인용, 동일 PR 번호 등 직접 참조는 없으므로 containment 아님
- 주장이 동일하지 않으므로 Duplicate 아님

**판정**: **Consolidate** / confidence Med. 같은 도메인 3건 누적, 메타 게이트 부재

**근거**: 같은 도메인("외부 가시성") 메모리 3건+ 분산. 진입점 메모리 부재로 발동률 저하와 중복 추가 위험
...
- Duplicate는 주장과 범위가 실질 동일/포함 (한쪽이 다른 쪽에 흡수 가능)
- Consolidate는 주장은 다르나 같은 도메인 분산 (진입점 메타로 묶음, 본문 유지)

**임계값 주의**:

- N은 3을 시작값으로 권장. 환경에 따라 조정. 단정형 적용 금지(예: "3건이면 무조건 Consolidate"). 하위 사실의 독립성과 발동 빈도 함께 고려

---

## 요약 패턴

- **확인 가능 사실 + 구체 출처**면 High confidence 판정 가능
- **부분 불일치**는 Outdated (Stale 아님)
- **검증 경로 매핑 실패**는 Stale (Low confidence)
- **관계 그래프 우선 활용**으로 Duplicate/Resolved 판정 정확도 향상
- **모순은 항상 사용자 확인**, 자동 처리 금지
- **같은 도메인 + 서로 다른 사실 N건+**면 Consolidate (Duplicate와 구분)
