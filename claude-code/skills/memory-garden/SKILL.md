---
name: memory-garden
description: Claude Code auto memory(MEMORY.md + 개별 엔트리 파일)를 주기적으로 정리합니다. 관계 그래프·confidence·근거 품질·dry-run·피드백 루프로 종료된 프로젝트·해소된 차단·반영 완료된 피드백·outdated 참조를 식별해 아카이브/삭제/갱신/통합하고, 메모리 작성 단계 개선 제안까지 생성. 프로젝트 문서 정원(`/garden`)과 타겟이 다름(메모리 엔트리 전용).
...
- 흐름: Phase 1 스캔 → 2 관계 그래프 → 3 판정 → 4 교차 검증 → 5 보고 → 6 실행(dry-run) → 7 정합성+피드백
...
```
Completed > Resolved > Promoted > Duplicate > Outdated > Stale > Active
```
...
4. 판정 의존 관계 있으면 보고에 명시(실행 순서 포함)
