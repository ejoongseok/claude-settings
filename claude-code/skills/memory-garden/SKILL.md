...
- **판정 근거는 구체 출처(`경로:줄 + 인용`) 필수**. 근거 품질 미달이면 Stale 강등
- **각 판정에 confidence(High/Med/Low) 필수**. Low면 Stale 강등
- **Stale 자동 처리 금지** (사용자 개별 확인). `user` 타입 갱신과 삭제는 명백 근거 필수. 아카이브 수정 금지
...
| **Claude Code auto memory 엔트리 정리와 개선 제안** | **이 스킬** | 핵심 |

## 외부 데이터 의존

| 데이터 | 경로 | 필수/선택 | 부재 시 동작 |
|--------|------|:------:|------------|
| auto memory 디렉터리 | system-reminder `# auto memory` 경로 또는 `~/.claude/projects/{profile-slug}/memory/` | **필수** | 경로 탐지 실패 시 사용자에게 AskUser로 직접 확인 후 중단 |
...
- 기본: 메인 문맥 일괄 처리. `Agent` 서브에이전트 위임 금지. 판정 근거(경로, 줄, 인용)의 연속성과 신뢰도가 서브에이전트 요약을 거치며 손실됨
- 경로는 POSIX 구분자(`/`) 표기. Windows 환경에서도 이 표기로 통일
- 흐름: Phase 1 스캔, 1.5 인덱스 그룹핑 점검, 2 관계 그래프, 3 판정, 4 교차 검증, 5 보고, 6 실행(dry-run), 7 정합성+피드백 순으로 진행
...
> - Bash: 각 파일 mtime 수집 (`stat` 사용. Linux `-c '%Y %n'`, macOS `-f '%m %N'`)
...
- 메모리 본문에서 언급된 경로와 심볼은 Phase 3에서 필요분만 Read/Grep
...
MEMORY.md가 평탄 리스트로 자라면 가독성과 발동률이 떨어진다. 임계 도달 시 카테고리 헤딩으로 그룹핑한다. 본문 변경 0건, 라인 재배치 + 헤딩 추가만.
...
구체 카테고리 라벨은 사용자 환경(역할과 도메인)에 따라 결정한다. 일반 골격 축:

- **type 축**: `user` / `feedback` / `project` / `reference` (4 헤딩, 가장 단순. 시작 권장)
- **활성도 축**: 활성 / 정체 / 메타 게이트(`★` 표지)
...
각 엔트리 본문과 description에서 **다른 엔트리 언급**을 식별 (파일명, 공통 주제, 동일 PR/이슈 번호, 날짜 연속성).

### 2-2. 관계 유형 분류

| 관계 | 정의 | 판정 힌트 |
|------|------|----------|
| **succession(선후)** | A가 B의 후속/갱신본 (예: PR 머지 단계별 메모리) | 구 엔트리는 Resolved/Completed 후보 |
| **containment(포함)** | A가 B를 부분집합으로 포함 (프로젝트와 그 내부 이슈) | 포함된 쪽은 Duplicate 후보 (상위 유지) |
| **contradiction(모순)** | A와 B가 상충하는 정보 | 양쪽 모두 Stale 또는 최신 기준 갱신 |
...
1. **claim 추출**: 본문에서 사실, 상태, 경로, 이름, 날짜 단위로 분리 (예: "PR #297 머지 완료", "담당은 역할명", "만료는 YYYY-MM-DD")
2. **검증 경로 매핑**: 각 claim의 확인 대상 결정 (STATUS.md / 소스 경로와 심볼 / CLAUDE.md 섹션 / 날짜 비교 등)
...
6. **판정 + confidence(High/Med/Low)**: Low면 Stale 강등

판정 과정과 근거 서술 방식은 [`templates/examples.md`](./templates/examples.md)의 few-shot 사례를 모범으로 삼는다.
...
| Outdated | 참조 경로, 코드, 상태 불일치 | claim vs 현재 상태 의미 비교. 경로와 심볼은 Grep/Read로 실체 확인 |
| Promoted | feedback이 CLAUDE.md/rules/skills에 의미론적 반영 | 반영처 Read 후 의미 매칭 판정 (grep 0건이어도 다른 표현일 수 있음) |
| Consolidate | 같은 도메인 메모리 N건+ 누적, 메타 게이트 없음 | 같은 본질의 서로 다른 사실들이 분산. Duplicate(주장 동일)와 구분. N 권장 시작값은 3, 환경에 따라 조정 |
| Duplicate | 다른 메모리와 주장과 범위가 실질 동일/포함 | 본문 의미 비교. Phase 2 containment 관계 활용. description 유사만으로 불충분 |
...
1. Duplicate ∩ Outdated이면 최신 상태 기준 병합
2. Promoted ∩ Active이면 Active 유지 + 본문에서 반영 부분만 제거(갱신)
3. Completed ∩ Resolved이면 Completed 우선
4. Consolidate ∩ Duplicate이면 Consolidate 우선 (메타 게이트 신규 + 하위 본문 유지로 정보 손실 0)
...
- 아카이브: 원경로에서 새 경로로 이동 (추가될 frontmatter 라인 포함)
- 삭제: 파일 경로 + 바이트 크기
...
| Consolidate | 메타 게이트 신규 + 하위 유지 | 진입점 메모리 신규 작성. frontmatter `meta_gate: true` + 본문에 하위 메모리 목록과 도메인 요약. 하위 N건 본문 유지(정보 손실 0). 인덱스에서 `★` 등 시각 표지 |
| Duplicate | 통합 | 한쪽에 병합 후 다른 쪽 아카이브. 두 본문 검토 후 재작성 |
...
- MEMORY.md 링크가 실제 `{auto memory}/*.md` 파일로 존재 (죽은 링크 0)
...
| feedback 반영 속도 | 작성부터 Promoted까지 평균 | 너무 길면 feedback 반영 체크 루틴 부재 |
...
- 피드백 루프 분석에서 `~/.claude/CLAUDE.md` 개정 제안 발생 시 `/optimize-claude-md` 또는 사용자 직접 편집
- `.local.claude/` 프로젝트 문서 정리가 함께 필요하면 `/garden`
- Stale 미결 건이 장기 누적되면 사용자 직접 검토 후 재실행

## 제약조건

- 아카이브 파일(`_archive/**`) 수정 금지 (역사 보존)
- Stale 자동 처리 금지 (사용자 개별 확인 대상)
- `user` 타입 갱신과 삭제는 명백 근거 필수 (환경 정보 낡음만 갱신 허용)
- Phase 6 Dry-run 미리보기 없이 실제 수정 금지
- 원본 메모리 파일을 Edit 할 때 frontmatter `type`/`name` 필드 변경 금지 (의미 변질 위험)
- 근거는 `경로:줄 + 인용` 형태 필수. 추상 서술("시스템 상태 확인" 등)은 Stale 강등
...
**[데이터 결함]** MEMORY.md와 개별 파일 간 죽은 링크 다수 (>10건)
