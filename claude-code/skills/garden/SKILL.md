...
```
Phase 1: 스캔 → Phase 2: 만료 식별 → Phase 3: 수확 점검
  → Phase 4: 보고 + 승인 → Phase 5: 실행 + INDEX 갱신
```
...
예: `retention: permanent` → 만료 없음, `retention: 7d` → 7일.
...
| daily-todos | (수확 불필요) | — | 작업 목록뿐, 스킵 |
| memo | 미승격 메모 | 사용자에게 질문 | /learn, /customer-profile 등으로 승격 여부 |
| draft | (수확 불필요) | — | 발신 콘텐츠, 스킵 |
| issues, prs | (수확 불필요) | — | GitHub에 원본, 스킵 |
| docs-parsed | `/absorb` 스킬로 처리 | — | `absorb-log.md` 확인: 흡수 완료 → 아카이브, 미흡수 → `/absorb {path}` 안내 |
| docs-original | (수확 불필요) | — | 바이너리, 스킵 |
...
|---|------|----------|----------|----------|
| 1 | daily/YYYY-MM-DD | (예시: 도메인 상태 전이 발견) | biz-rules.md | § 해당 상태 |
...
반영 시 반드시 Read → Edit 순서로 최신 상태 확인 후 수정.
...
### 아카이브
- 이동: N건 → archive/YYYY-MM/
- docs: N건 → archive/docs/

### 현재 활성 문서: N개 (아카이브 전 N개 → N개로 감소)
...
- docs/original은 .docx, .xlsx 등 바이너리 — 수확 없이 이동만
