---
name: data-diagnosis
description: 데이터·메트릭 관점 진단 — 핵심 지표 정합성, 데이터 신뢰도, 분석 커버리지, 실험 문화, 셀프서비스 성숙도, 데이터 거버넌스. "측정하지 않으면 개선 못 한다" 의 인프라 점검.
...
4. **추정 → 실측 전환 권고.** "DAU 10000 정도" 보다 "DAU 정확히 12,347" 가능한가
...
| 소스 | 신뢰도 | 데이터 관점에서의 가치 |
|------|:------:|-----------------|
| 분석 코드 (Grep) | ★★★★★ | 객관 — 무엇을 측정하는지 |
| 메트릭 정의 문서 | ★★★★ | 합의 수준 |
| 대시보드 (사용자 제공) | ★★★★ | 실제 활용 흔적 |
| 분석 도구 설정 | ★★★★ | 수집 범위 |
| biz-rules.md | ★★★ | 비즈니스 정의 (지표 정의 충돌 검출) |
| daily/ (분석 활용 신호) | ★★ | 누가 데이터로 의사결정 하는가 |
...
4. **권고 우선순위**: P1 (정합성·신뢰) > P2 (커버리지) > P3 (셀프서비스·거버넌스)
```
...
- 지표 정의 충돌 → `/biz-rules` 로 정의 합의 + 문서화
- 부재 지표 추가 → `/srs` 로 트래킹 요구사항 정의
- 분석 도구 도입 → `/prd` 로 도구 평가
- 거버넌스·권한 → `/security-diagnosis` 연계
...
- **권고 우선순위 명확.** 거버넌스부터 손대면 효과 늦음 — 정합성·신뢰 먼저
...
- 신호: 같은 이름(예: MAU, revenue)을 서로 다른 로직·필터로 계산
- 대응: 양쪽 정의를 병기 + `[데이터 충돌]` 태그 + SSOT 합의 필요 플래그

**[도메인 특수성]** 단일 테이블·단일 소스만 존재
- 신호: DB 1개 / 테이블 수 소수 / 파이프라인 부재
- 대응: SSOT·lineage 분석 대신 테이블 구조 점검(스키마·PK·인덱스)만 수행
