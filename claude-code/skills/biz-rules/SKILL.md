...
| 질문 | 담당 | 이 스킬에서 |
|------|------|-----------|
| 비즈니스·상태 전이·도메인 점검 카테고리 문서를 작성·갱신 | **이 스킬** | 핵심 — 도메인 지식 SSOT |
| 이 문서를 진단 루브릭으로 소비 (아키텍처·기술 진단) | /tech-diagnosis, /analyze-dir | 생산만 — 진단·해석은 위임 |
| 코드 변경의 도메인 버그 점검에 §4 카테고리 활용 | /review | 카테고리 제공만 — 리뷰 판정은 위임 |
