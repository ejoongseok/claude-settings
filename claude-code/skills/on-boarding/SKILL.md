...
| 질문 | 담당 | 이 스킬에서 |
|------|------|-----------|
| "신규 프로젝트 6산출물 자동 생성" | **이 스킬** | ✓ 핵심 (CLAUDE.md, bot/, human/, biz-rules 와 team 스켈레톤, SETUP.md) |
| "이미 만든 SETUP.md 점검, 갱신, 암묵지식 누적" | /setup-guide | 다루지 않음 (생성은 이 스킬, 이후 유지는 setup-guide) |
| "디렉터리 구조만 스캔과 집계" | /analyze-dir | 다루지 않음 (이 스킬은 산출물 생성까지 수행) |
...
- `--focus backend` 는 서버 사이드 코드 및 API 중심 분석
- `--focus frontend` 는 클라이언트 사이드 및 UI 중심 분석
- `--focus api` 는 API 엔드포인트 및 통합 중심 분석
- `--focus database` 는 데이터 모델 및 스키마 중심 분석
- `--focus testing` 은 테스트 전략 및 커버리지 중심 분석
- `--focus devops` 는 배포 및 인프라 구성 중심 분석
- `--format summary` 는 간략한 요약만 제공
- `--format detailed` 는 상세 분석 제공 (기본값)
...
판별 기준:
- pom.xml 또는 build.gradle 이면 Java/Kotlin (Spring Boot, Quarkus 등)
- package.json 이면 JavaScript/TypeScript (React, Vue, Next.js, Express 등)
- requirements.txt 또는 pyproject.toml 이면 Python (Django, FastAPI, Flask 등)
- go.mod 이면 Go
- Cargo.toml 이면 Rust
- *.csproj 또는 *.sln 이면 .NET (C#)
- Gemfile 이면 Ruby (Rails 등)
- composer.json 이면 PHP (Laravel 등)
- 복수 빌드 파일이면 모노레포 또는 멀티 스택 (각각 분석)
...
> - 로드: 4개 영역(A 도메인과 데이터, B API 와 요청 흐름, C 외부 연동과 인프라, D 테스트와 품질) 의 핵심 파일을 동시에 Read: 엔티티 모델, 대표 컨트롤러/라우터, 외부 연동 설정, 테스트 샘플
...
한 번에 모든 영역을 분석하지 말고, 영역별로 분석과 저장을 마친 뒤 다음 영역으로 넘어가는 순서를 따르세요.
...
영역 B: API 및 요청 흐름
- 주요 API 엔드포인트 목록
- 대표 API 하나의 전체 흐름 추적 (진입, 처리, 응답 순)
- 인증/인가 체계
...
2. **bot/INDEX.md + bot/*.md** (프로젝트 루트의 bot/ 디렉터리): AI 사실 카탈로그
   - bot/INDEX.md 는 라우팅 진입점 (각 bot 파일의 용도와 갱신일 표)
   - bot/*.md 는 정보량에 따라 1~5개 분할 (작은 프로젝트는 1~3개로 충분)
...
3. **human/README.md** (프로젝트 루트의 human/ 디렉터리): 사람 진입점
   - 3단 구조: 1. 배경 / 2. 현재 상태 (Mermaid 다이어그램 1+ 필수) / 3. 남은 질문과 의사결정
   - "더 읽을 거리" 섹션에 bot/INDEX.md, CLAUDE.md 안내

4. **`.local.claude/biz-rules.md` 스켈레톤** (도메인 점검 카테고리 진입점): 빈 템플릿 자동 생성
   - 신규 프로젝트는 도메인 규칙과 점검 카테고리가 아직 없으므로 작업하며 점진 채움
   - `/review`, `/srs`, `/qa`, `/briefing`, `/deploy-checklist` 등이 4절 (도메인 점검 카테고리) 를 자동 활용
   - 스켈레톤만 생성, 사용자 안내: "이 파일 4절 (도메인 점검 카테고리) 를 채울수록 다른 스킬의 도메인 점검이 강화됩니다"
...
6. **`human/SETUP.md`** (개발 환경 셋업 가이드): 신규 합류자가 처음부터 따라하면 환경이 준비되는 가이드
   ...
   - **구조 (필수 섹션)**:
     ...
     ## 6. 첫 실행                  # 의존성 설치, 빌드, 개발 서버, 검증 명령 순
   ...
   - 코드/설정에서 감지 안 된 항목은 섹션 통째로 생략 (빈 placeholder 금지). 단 7절 (사람에게 들어야 할 것, 암묵 지식) 은 항상 포함
   - 사용자 안내: "이 파일은 살아있는 문서입니다. 환경 변경이나 트러블슈팅 발견 시 `/setup-guide` 로 갱신하세요."
...
| 산출물 | 임계 | 분리 시 패턴 |
|--------|:----:|------------|
| `CLAUDE.md` (자동 로드) | 200줄 | optimize-claude-md 가 검사와 분리 (기존) |
...
- 6산출물 생성 완료 후에는 `/setup-guide` 로 SETUP.md 환경 점검과 갱신
- biz-rules.md 스켈레톤 채우기는 `/biz-rules` (도메인 규칙 발견 시)
- team.md 스켈레톤 채우기는 `/people` (동료 프로필), `/customer-profile` (B2B 고객)
- 계약 준수 첫 점검은 `/skill-check`
- 다음에 무엇을 할지 모를 때는 `/skills-guide --new`
- 산출물 비대와 만료 정리는 `/garden`

## 제약조건

- **민감 정보 마스킹**. API 키, 비밀번호, 토큰은 문서와 중간 파일(.claude/tmp/)에 포함하지 않습니다. .env 는 키 이름만, 값은 마스킹.
...
- **본문 도메인 가정 금지**. 도메인 규칙은 biz-rules.md 스켈레톤으로 위임 (CONTRACT '도메인 가정 외부화 원칙' 절).
