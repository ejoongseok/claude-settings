...
> 이 파일은 `/session-handoff`가 생성하는 인계 문서의 **예시**다 (일반 가상 도메인인 주문 API). 실제 자격증명과 내부 정보 없음. 양식의 SSoT는 `SKILL.md`의 템플릿 절이며, 이 예시를 양식 표준으로 삼지 않는다.

## 다음 세션 모델 지침

read 후 순차 수행:
1. frontmatter `work_type` (debug) + `edge_cases_detected` (#2 dirty git) 분기 식별
2. "진입 read 의무" 절 모든 절대 경로 read (병렬 가능)
3. "환경 상태" 절 미커밋 변경 + 중단된 부하 테스트 재연결 확인
4. "stale 검증" 절 자가 점검 1회
5. "다음 액션 우선순위" 절 1번(잔여 가설 H3 검증) 진입
...
| 4 | external | logs/order-api-2026-06-01.log | A/B | 500 발생 시각과 스택 raw |
...
미커밋 3건: OrderService 타임아웃 로깅 추가 + 풀 size 실험값(10에서 20으로) + 부하 테스트 스크립트(신규, untracked).
...
| src/main/resources/application.yml | max-pool-size 10에서 20으로 (실험, 커밋 보류) | tracked |
...
| # | 발견 / 단정 | 정정 / 검증 |
|:---:|---|---|
| 1 | 초기 단정 "DB 데드락" | 정정. 데드락 로그 0건. 스레드 덤프상 다수 스레드가 `getConnection` 대기여서 풀 고갈(H3)로 방향 전환 |
...
| DB 쓰기 | 없음 (조회와 재현만, 롤백됨) |
| 미커밋 | application.yml 풀 size 실험값 20 (커밋 보류. H3 검증 후 결정) |
| 로그 자격증명 | application.yml DB URL에 평문 비밀번호 노출. 본 문서와 로그 인용 시 `[REDACTED-PWD]` 처리 |

## 다음 액션 우선순위

### 1. H3(풀 고갈) 확정 검증 (가치 3/3)

| 항목 | 값 |
|---|---|
| 진입 path | scripts/load-test-order.sh 실행 + 풀 대기와 활성 커넥션 메트릭 관찰 |
| 가치 | 3/3 |
| 부작용 | 없음 (로컬 부하만, DB 롤백) |
| 예상 소요 | 30분 |
| 관련 참조 | "진입 read 의무" 절 #2 |

### 2. 수정 방향 결정 (가치 2/3)

풀 size 상향(임시) vs 트랜잭션 구간 단축(근본). H3 확정 후 "사용자 트리거 대기" 절 참조.

## stale 검증 (재진입 시 1회)

| 항목 | 검증 명령 | 갱신 가능성 |
|---|---|---|
| git HEAD | `git log -1 --format=%H` ≠ a1b2c3d 이면 진행됨 | high |
