...
본 세션 종료 시점에 호출되어, **다음 세션의 모델**이 컨텍스트를 재구성할 수 있도록 인계 문서를 작성한다. 인계 문서는 사람이 아닌 **다음 세션 모델 독자 기준**으로, 명령형 지시문 + frontmatter 메타 + structured data 위주. 콘솔 출력만 사람 친화 한국어로.

## 컨텍스트

- **호스트**: large-context 모델 가정. 본 대화 처음~끝 전체를 한 번에 흡수 가능. 더 작은 컨텍스트 모델로 새 세션 진입 가능성 시 분량 경고 자동 삽입.
- **호출**: 사용자 명시 (`/session-handoff [topic]`). `disable-model-invocation: true` 이므로 자동 호출 X.
- **작업 유형 10종**:
  ...
  - `exploration`: 대규모 코드베이스 탐색과 모듈 분석
  - `meta`: 스킬, hook, 설정, 메모리 시스템 작성 (본 스킬 자체가 meta 사례)
  - `retro`: 회고, 메모리 정리, 작업 마무리
  - `mixed`: 2개 이상 50%+ 동시 매치
  - `unknown`: 신호 약함 (사용자 한 줄 확인 후 진행)
- **저장**: 작업 유형별 자동 경로 분기 ('자동 추론 경로' 절). `LATEST.md` 매번 갱신, `INDEX.md` append.
- **독자 분리**:
  - 인계 문서는 다음 세션 모델용: 명령형 + structured data
  - 콘솔 출력은 사람용: 한국어 친화 + 사용자 입력 명령어 안내

## 외부 데이터 의존

| 데이터 | 경로 | 필수/선택 | 부재 시 동작 |
|--------|------|:------:|------------|
| 본 대화 컨텍스트 | (대화 내, 채널 A) | 필수 | 흡수 대상 없으면 인계 가치 낮음, 사용자 확인 후 진행 |
| git 상태 | `git status` / `git log` / `git symbolic-ref HEAD` | 선택 | git 아니면 frontmatter `N/A`, 환경 상태 섹션에 명시 |
| 진척 추적 파일 (sprint) | 프로젝트별 진척 추적 문서 | 선택 | 부재 시 sprint 는 `mixed` 로 강등 (엣지케이스 #3) |
...
Retention 디폴트: sprint, meta, retro 는 permanent / debug 는 14d / pr, doc, outreach, exploration 은 7d.

## 작업 유형 신호 매트릭스 (Step 1)

| 유형 | 트리거 (본 대화 / git / 디렉터리 시그널) |
|---|---|
| sprint | 진척 추적 파일 read와 write / feature 브랜치 / 진척률, 잔여 키워드 |
| pr | `gh pr ...` / github MCP 도구 / 본 세션 commit + push / `.github/` 활동 / PR 번호 언급 |
| debug | error trace 다수 / 테스트 반복 실패 / 가설, 재현, 진단 키워드 / stack trace paste |
| doc | `.md` 작성과 갱신 50%+ / docs/ 활동 / 섹션, 목차 |
...
| mixed | 2개 이상 50%+ 매치 |
| unknown | 신호 약하면 사용자 한 줄 확인 |

복수 매치 시 `mixed`. 호출 인자 `[topic]` 이 유형 hint 포함 시 우선.
...
**작업 유형별 디폴트 참조 자산** (셀은 참조 *종류*다. 자신의 메모리와 문서에서 해당 종류를 찾는다):
...
| meta | 스킬 작성, 외부 공개 관련 메모리 + 본 대화 read | 대상 `SKILL.md`(참조 스킬), 설정 파일, `CLAUDE.md`, 룰셋 | (해당 시) | 스킬과 hook 도구 호출 대상 | (해당 시) |
...
| 2 | git detached / merge / dirty | `git status --porcelain` + `git symbolic-ref HEAD` | frontmatter `detached@{SHA}` / `merge:{base}..{head}` + "환경 상태" 절 미커밋 인벤토리 |
...
| 7 | 자산 50+ | Step 3.5 카운트 | 우선순위 정렬 후 상위 10개는 "진입 read 의무" 절, 나머지는 "관련 참조 (추가)" 절 |
...
- **엣지케이스 #2 (git 비정상)**: `git status --porcelain` + `git symbolic-ref HEAD`. detached / merge / dirty 검출 시 frontmatter 표기 + "환경 상태" 절 미커밋 인벤토리 작성.
- retention 결정 (작업 유형 디폴트 적용).

### Step 1: 작업 유형 자동 식별

"작업 유형 신호 매트릭스" 절의 10종 적용. 호출 인자 `[topic]` 안에 유형 hint (예: `meta-foo`, `debug-bar`) 있으면 우선.
...
- 호출 인자 우선
- 미지정 시 작업 유형 + 컨텍스트로 자동 추론 (예: sprint 이면 가장 최근 read 한 진척 추적 파일의 작업명)
- **엣지케이스 #4 (특수문자와 충돌)**: `[topic]` sanitize: `/\?*<>:\|"` 제거 + 공백은 `-` 로 치환. 같은 분 다중 호출 시 파일 존재 확인 후 `-2`, `-3` 자동 suffix.
- 경로 결정 ("자동 추론 경로" 절 적용)
- 디렉터리 생성:
...
- 작업 유형별 추가 read:
  ...
  - meta: 본 세션 작성과 검토 대상 `.claude/skills/*/SKILL.md` 또는 설정 파일 등
  - retro: 본 작업 feedback, reference 류 메모리, 회고 문서

**엣지케이스 #10 (재귀)**: 본 대화에 `category: handover` frontmatter 인계 문서 read 흔적 있으면 raw 재포함 금지. type external 절대 경로 참조만 "진입 read 의무" 절에 포함.
...
**엣지케이스 #7 (자산 50+)**: 카운트 ≥ 50 시 상위 10개만 "진입 read 의무" 절에 포함, 나머지는 "관련 참조 (추가)" 별도 섹션.

각 자산에 type 컬럼 분류 (7종):
- `session-output`: 본 세션 신규 생성 .md / .json / 이미지 등
- `memory`: 사용자 메모리 (feedback, reference, user, project 류)
- `convention`: 행동 지침과 룰셋 (`CLAUDE.md`, `rules/*.md` 등)
- `domain-doc`: 프로젝트 도메인 룰 (도메인 규칙, 모듈, 고객 문서 등). convention 과 구분
- `sprint-ssot`: 진척 추적 파일 등
- `code-ref`: 코드 파일:줄번호
- `external`: 학습 노트, 외부 paste 원본, 다른 인계 파일 등

**복수 채널 매치 표기**: 한 자산이 A/B/C 중 2 이상에 매치 시 `A/B` 또는 `A/B/C` 다중 표기 (강한 채널 앞). 예: 본 대화 직접 read + 사용자 명시 둘 다면 `A/B`.
...
| meta | 작성과 검토 대상 N, 변경 의도 (신규 도입 / 보강 / 폐기 / 통합), 영향 범위 (본 / 글로벌 / 양쪽), 검증 방법 |
| retro | 회고 범위, 정리 대상 N, rename/merge/delete N, 일반화 룰 도출 N |

### Step 5: 발견, 정정, 결정 자동 추출

본 대화 흐름에서:
- **단정 정정 사이클**: 사용자 "X 가 아니라 Y" / "정정해" / "다시 봐" 추적 후 표로 정리
- **신규 메모리**: 본 세션 생성된 메모리와 학습 노트 인벤토리
...
- DB INSERT/UPDATE 영역 (테이블과 키값)
- 외부 컨텍스트 (회신 대기, 회의 일정)
- 미완 상태 (테스트 중단, build 실패, git 미커밋)

### Step 7: 다음 액션 우선순위 정렬

**"다음 액션" 절 정의**: 새 세션 모델이 **단독 진입 가능한 작업**만 포함. 사용자 결정이나 입력 대기 항목은 "사용자 트리거 대기" 절로 분리. 둘 중 한 곳에만 등재 (중복 금지). 분리 기준:

- 모델이 컨텍스트만으로 즉시 시작 가능하면 "다음 액션" 절
- 사용자 응답 (Y/N, 옵션 선택, 외부 자료 paste 등) 없이 시작 불가면 "사용자 트리거 대기" 절
...
- 부작용과 리스크
- 예상 소요 (5분 / 30분 / 1시간 / 반일)
- 관련 참조 ("진입 read 의무" 절 # 번호)

작업 유형별 디폴트 우선순위:
- sprint: 잔여 카테고리 진척 가치 순
- pr: 미응답 리뷰, CI 실패, conflict, 머지 순
- debug: 잔여 가설, 환경 변수, 회귀 순
- doc: 미완 섹션, 의사결정, 외부 공유 순
- meta: 검증 시나리오, 사용 후 평가, 보강 순
- retro: 일반화 룰 도출, 메모리 정리, 후속 액션 순

### Step 8: 인계 문서 작성 (템플릿 절 준수)

**0건 섹션 처리 분기 룰**:
- **이전 호출에 데이터 있던 영역이 본 호출 0건**이면 한 줄 명시 ("0건 (직전 N건)" 형식). stale 감지 가치가 있다 (예: 직전 sprint 진척 N건이 본 호출 0건이면 진척 정체 신호)
- **본 세션 주제와 무관한 영역 0건**이면 섹션 통째 생략 (noise 회피)
...
`LATEST.md` 내용은 Write 로 갱신 (Append X):
...
- [ ] "다음 세션 모델 지침" 절 명령형 톤 (안내문, 강조어 0건)
- [ ] "진입 read 의무" 절 type 컬럼 7종 (session-output/memory/convention/domain-doc/sprint-ssot/code-ref/external) 중 1개로 모두 분류
...
- [ ] "진입 read 의무" 절 상위 10개 이하 (51+ 시 별도 섹션)
...
- [ ] "다음 액션" 절 vs "사용자 트리거 대기" 절 **중복 등재 0건** (모델 단독 진입 가능 / 사용자 입력 필요 중 양자택일)
- [ ] "0건 명시" 항목은 직전 회차 데이터 있던 영역만 (무관 영역 0건은 통째 생략. 분기 룰 적용)
- [ ] 표와 코드 블록 앞뒤 빈 줄 (CommonMark, grep 부분 read 효율)
- [ ] 빈 섹션 0건
- [ ] 이모지 0
...
**엣지케이스 #8 (더 작은 컨텍스트 모델 가능성)**: 사용자 명시 또는 환경 시그널 시 인계 문서 "다음 세션 모델 지침" 절 첫 줄에 분량 경고 자동 삽입:

> 본 문서 분량 {N}줄. 작은 컨텍스트 모델은 "진입 read 의무" 절 상위 5개만 read 권장.

콘솔 출력은 "콘솔 출력 양식" 절을 따른다.
...
데이터 없는 섹션 통째 생략. 명령형 + structured data. 사람 친화 안내문과 강조어 금지. 코드 블록과 표 앞뒤 빈 줄 의무 (CommonMark, grep 부분 read 효율).
...
edge_cases_detected: ["#N", "#M"]  # YAML 안전 형식. 빈 배열은 [] / 상세 설명은 본문 "환경 상태" 절
---
...
2. "진입 read 의무" 절 모든 절대 경로 read (병렬 가능, type 우선순위는 sprint-ssot, session-output, memory, convention, domain-doc, code-ref, external 순)
3. "환경 상태" 절 활성 세션 재연결 가능성 확인
4. "stale 검증" 절 자가 점검 1회
5. "다음 액션 우선순위" 절 1번 진입 또는 "사용자 트리거 대기" 절 참조
6. 발견/정정/결정 표는 raw 기록 (재실행 X)
...
식별 채널: A는 본 대화 직접 read, B는 사용자 명시, C는 메모리 인덱스 grep.
...
| 항목 | 검증 명령 | 갱신 가능성 |
|---|---|---|
| git HEAD | `git log -1 --format=%H` ≠ {source_session_last_commit} 이면 N 커밋 진행 | high |
| 환경 세션 | 브라우저/MCP 세션 종료 시 재로그인 필요 | high |
| 진척 추적 파일 mtime | mtime 변경 시 다른 세션 진행 가능성 | medium |
| 메모리 인덱스 mtime | 변경 시 신규 메모리 가능성 | medium |
| 인계한 메모리 mtime | 각 메모리 파일 mtime 변경 | low |
| 인계한 도메인 문서 mtime | 변경 시 도메인 룰 갱신 | low |
...
| 필드 | 값 |
|---|---|
| 작성과 검토 대상 | {SKILL.md / 설정 파일 / CLAUDE.md / hook / rules} |
...
- **양식 SSoT는 "다음 세션 모델 독자 기준 인계 문서 템플릿" 절** (본 SKILL.md 내부)이다. 외부 어떤 인계 파일도 양식의 기본이나 표준 아님.
- 본 대화나 사용자 발언에 특정 인계 파일이 등장해도 그것은 **일회성 사례**. 채널 A (본 대화 직접 read) 로 자동 식별돼도 "참조 자산" 으로만 활용하고 양식과 분량의 기본으로 일반화 금지.
- 분량은 본 세션 컨텍스트 양에 비례한다. 짧은 대화는 짧은 인계 (50줄 가능), 큰 작업은 풍부 인계 (500줄+ 가능). 특정 사례의 풍부도를 모든 인계의 기본으로 매핑 금지.
- 양식 추가와 변경은 본 SKILL.md 의 템플릿 절 갱신을 통해서만. 외부 참조 파일 갱신으로 양식 우회 X.

## 독자 분리 원칙

- **인계 문서 독자는 다음 세션 모델**: 명령형 + frontmatter + structured data. 사람 친화 안내문과 강조어 금지.
- **콘솔 출력 독자는 사람**: 한국어 친화 톤 + 사용자 입력 명령어 안내.
...
| auto memory 엔트리 정리와 아카이브 | `/memory-garden` | [다루지 않음] |
| 다음 세션 모델용 인계 문서 작성 (휘발 컨텍스트 보존) | **이 스킬** | [핵심] |

## 사람용 사용법

README.md 참조. 결과물 예시는 `examples/` 에 있다. 사람이 산출물 형태를 미리 보기 위한 것이다. 양식 SSoT는 본 문서의 템플릿 절이며, 예시를 양식과 분량의 표준으로 삼지 않는다 ("양식 SSoT 원칙" 절).
