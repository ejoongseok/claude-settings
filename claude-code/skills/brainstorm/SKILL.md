...
| 사실 카탈로그/아키텍처 | `bot/INDEX.md` 또는 `.local.claude/INDEX.md`/`ONBOARDING.md` | 선택 | 모듈 매핑·이벤트 흐름 제약 확인 생략 |
...
부재 시에도 발산·수렴은 수행 — 코드베이스 근거가 없으면 `[추정]` 태그로 표기.
...
| "어떤 모듈에 해당하는지?" 확인 | `bot/INDEX.md` 또는 `.local.claude/ONBOARDING.md` 의 핵심 사용자 여정↔모듈 매핑 |
...
| 유형 | 설명 | 접근 |
|------|------|------|
| **문제 해결** | "고객이 ~한 문제를 겪고 있다" | 문제를 재정의 → 해결 방향 발산 → 평가 |
| **기술 의사결정** | "A 방식 vs B 방식" | 선택지 명확화 → 평가 기준 설정 → 비교 |
| **아이디어 발산** | "이런 걸 해보면 어떨까" | 아이디어 확장 → 그루핑 → 우선순위 |
| **정리/구조화** | "얘기가 많이 나왔는데 정리가 안 돼" | 토픽 추출 → 관계 정리 → 다음 단계 도출 |
...
> 1. 근거 재점검 — 각 방향의 장/단점·소요·리스크가 프로젝트 코드(modules, ADR)에 근거하는가? 추측·일반론은 [추정] 태그가 있는가?
> 2. 전제 검증 — 추천안이 유효하려면 어떤 전제(기술 스택, 팀 역량, 비즈니스 제약)가 성립해야 하는가?
> 3. 반대 증거 — "사용자가 놓친 대안·관점(비기술적 경로, 인접 모듈 재사용, 외주/SaaS 대안)은 없는가?" 반박 1개 이상 후 반영
