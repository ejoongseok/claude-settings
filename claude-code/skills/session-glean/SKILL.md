---
name: session-glean
description: "현재 세션에서 발견한 새 지식·결정·교훈을 작업 디렉터리 문서에 반영합니다. 관련 문서가 있으면 갱신하고, 없으면 적합한 위치에 신규 문서를 생성합니다. 세션 중간·말미에 호출하여 학습을 휘발시키지 않고 문서 자산으로 보존하세요."
...
호출 전 자동 read: `CLAUDE.md`, `bot/INDEX.md` 또는 `.local.claude/ONBOARDING.md`, `.local.claude/docs/absorb-log.md`. 모두 부재 → `/on-boarding` 안내 후 종료.

## 모드 판단

| `--scope` | 동작 |
|---|---|
| `auto` (기본) | `.local.claude/` 존재 → `local-claude`, 부재 → `cwd` |
...
존재하지 않는 경로 → 종료. 파일 지정 → 그 파일만 sink. 디렉터리 지정 → 하위만 스캔.
...
| 결정(Decision) | 합의·지시된 방향 | "이 모듈은 A 패턴으로 통일" |
| 규칙(Rule) | 반복 적용 규칙 | "PR 시 체크리스트 N 항목 필수" |
| 관계(Relation) | 엔티티 간 연결 | "C 모듈 → D 테이블 의존" |
...
| 통과 | 폐기 |
|---|---|
| 반복 적용 규칙·정책 | 일회성 작업 현황 |
| 구조적 변경 | 단순 질의응답 |
| 결정 + 근거 | 코드·git에 이미 기록 |
| 상태 전이, 인원 변동, 마일스톤, 고객사 특성 | 개인 감상, 일정 조율 |

5. 영속 항목 0개 → "영속 학습 추출 안 됨" 안내 후 종료. 빈 보고서 미생성.
...
- 후보 12+ → 상위 12 + "추가 N개" 안내
- 후보 0 → Phase 3 신규 위치 결정
...
1. SKIP — Grep 동일·유사 표현
2. CONFLICT — 기존 sink 모순
3. 분량 임계 — 변경 후 줄 수 추정 (CONTRACT §5 표)

신규 파일 위치:

- `bot/` 존재 → `bot/{topic}.md`
- `docs/` 존재 → `docs/{topic}.md`
- 둘 다 부재 → `docs/` 신규 생성 후 `docs/{topic}.md`
- `local-claude` 모드 → `.local.claude/{modules,customers,...}/`
...
2. 처리 순서: CONFLICT → UPDATE → ENRICH → NEW → CREATE_FILE
3. Edit는 한 문서·한 섹션 단위
4. 신규 파일 frontmatter (CONTRACT §7-2 참조): `category: {topic}, retention: {sink 도메인 기본값}, harvest_targets: [...]`. 첫 줄 `> /session-glean 으로 생성 — YYYY-MM-DD`
...
- 신규 파일: CONTRACT §7-2 frontmatter + 첫 줄 `> /session-glean 으로 생성 — YYYY-MM-DD`
- 기존 갱신: 변경 이력 테이블 있으면 `| YYYY-MM-DD | session-glean: {요약} | 세션 §N |` 추가
...
CONTRACT §5 전수 준수. Phase 3 사전 측정.
...
| 만료·아카이브 | `/garden` | [다루지 않음] |
| 자동 메모리 정리 | `/memory-garden` | [다루지 않음] |
| 단건 코드 검증 | `/learn` | [다루지 않음] |
| AI/사람 레이어 재구성 | `/dualize-docs` | [다루지 않음] |
| 프로젝트 진입 산출 | `/on-boarding` | [다루지 않음] |
| 세션 학습 → 문서 증분 반영 | 이 스킬 | [핵심] |

## 다음 스킬 연결

- `CLAUDE.md` 200줄 근접 → `/optimize-claude-md`
- 외부 디렉터리 흡수 필요 → `/absorb {path}`
- 만료 정리 시점 → `/garden`
- 신규 모듈·고객사 정의 → `/briefing` 또는 `/customer-profile`
- 단건 claim 코드 검증 → `/learn`
- 데이터 부재 → `/on-boarding`
