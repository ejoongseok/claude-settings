...
기대값 분류에서 **Deep 만의 차이**:
- **natural-language**: `ai-vision-check` — 스크린샷 + 기대값을 multimodal 이 직접 판정 (§5.3)
...
> **[메타인지]** 핵심 결론 Top 3 (P1 액션·코드↔QA 일관성 구멍·시맨틱 우선순위 재산정)에 대해:
...
| 경량·빠른 MCP 실행 + 7카테고리 리포트 | `/qa-run-light` | [다루지 않음] |
...
- 신호: 대상 코드베이스·문서 합산 추정 토큰 ~600K+ (합산 라인 수로 대략 가늠)
