...
### 8.12 연속 클린 시 실행기 강등 제안

**트리거**: 7.2절 회귀 트렌드 데이터 기준, 이번 run 포함 최근 3회 연속으로 회귀 0 + AI 시각 판정 fail 0 + 사용자 개입 0. QA frontmatter `recommended-runner` 가 `deep` 일 때만.

**출력 예**:
```
- **[확신][P2]** 최근 3 run 연속 클린 (회귀 0, 시각판정 fail 0, 개입 0). Deep 유지 근거가 소진되어 `recommended-runner: light` 강등 제안
  - 근거: {run-id 3건} 전 Step OK + 7.2절 회귀 트렌드
  - 액션: 다음 run 은 `/qa-run-light` 사용. QA 재생성(`/qa <원본>`) 또는 frontmatter 수정은 사용자 결정
  - 공수: 5분
```

**규칙**: 제안만 출력한다. QA-*.md frontmatter 를 이 스킬이 직접 수정하지 않는다 (원본 불변). 리스크 변화(신규 기능, 대규모 리팩터링, 자연어 기대값 증가) 시 Deep 복귀 판단은 `/qa` 6.1절 재결정과 1.1절 추천 모드 확인이 담당.
...
- **인사이트 비어 있을 수 있음**: 8.13절 인사이트 부재 처리로 정직하게
