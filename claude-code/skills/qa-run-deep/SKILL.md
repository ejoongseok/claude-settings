---
name: qa-run-deep
description: "QA-*.md 체크리스트를 MCP Playwright 로 실행하면서 관련 SRS·PRD·구현 코드·biz-rules·git log·과거 run 을 1M context 에 동시 맥락화. AI 시각 판정·예측 QA·공통 원인 클러스터링·메타-QA·코드↔QA 일관성·시맨틱 우선순위 재산정·대안 비교까지 수행. 비싸므로 추천 신호 충족 시에만."
...
- `deep` → 그대로 진행
- `light` → AskUserQuestion: "이 QA 는 light 권장 ({recommended-runner-reason}). Deep 으로 진행하면 시간·비용 증가. 그래도 진행?"
...
(`/qa-run-light` §2.2~§2.4 와 동일)

기대값 분류에서 **Deep 만의 차이**:
- **natural-language**: `ai-vision-check` — 스크린샷 + 기대값을 Opus 4.7 multimodal 이 직접 판정 (§5.3)

### 2.3 수동 Step / 우선순위 / 도메인 (§2.5 와 동일)

### 2.4 파서 매치율

Step 헤더 매치율 < 80% → AskUserQuestion (재생성/강행/중단).

---

## 3단계: Deep 맥락 수집

> **[Opus 4.7 / 1M 활용]** 서브 분할 없이 메인에서 직접 **다중 파일 동시 로드** (L80~98 가드레일 강화):
...
**코드 축**:
- QA Step 에 언급된 경로 (controller·service·mapper) → Grep 으로 추출 후 Read
- 없으면 Step 제목 키워드로 `Grep -r <키워드> {프로젝트 코드 디렉터리}` → 상위 3개 파일 Read

**히스토리 축**:
- `git log --since={since-days}.days.ago -- {관련 경로}` → 커밋 해시·메시지·변경 파일
- `Glob {dirname(QA)}/QA-RUN-*.md` 후 mtime 으로 {since-days}일 필터 → 과거 run N개 Read

**맥락 총량 가드**: Read 한 파일 총 줄 수 600,000 줄 초과 추정 시 AskUserQuestion ("범위 축소: 7일 / 14일 / 유지 / 중단").

### 3.1 예측 QA (실행 순서 재정렬)

수집한 git log + 과거 run 으로 **이번 run 의 위험 Step** 예측:

- 지난 {since-days}일 커밋이 건드린 파일 집합 ∩ 각 Step 이 간접 참조하는 도메인 → 교집합 높은 Step 에 `predicted-risk: high` 태그
...
- Step ≤ 20 + 맥락 적음: 진행
- Step 20~50: 로그만
- Step > 50 또는 맥락 600K 줄 근접: AskUserQuestion ("Deep 진행 / Light 전환 / `--only P1` / 중단")
...
3. assertion 비교:
   ...
   - `natural-language`: **`ai-vision-check`** — §5.3
...
### 5.2 Retry (§5.4 막힘 감지 우선)

Light 와 동일. 단, 막힘 감지 신호 시 retry 즉시 중단 (§5.4 가 우선).

### 5.3 AI 시각 판정 (Deep 핵심 차이)

자연어 기대값을 가진 Step:
1. Step 종료 시 `browser_take_screenshot` 으로 화면 캡처 (이미 증거 계층에 포함)
2. 이 스크린샷을 **Read 도구로 메인 컨텍스트에 올림** (Opus 4.7 multimodal)
3. 기대값 자연어와 비교하여 판정:
...
(`/qa-run-light` §5.4 와 동일)

### 5.5 stop-on-fail + 격리

(`/qa-run-light` §5.3 과 동일)
...
지난 {since-days}일 모든 `QA-RUN-*.md` 조인 → 트렌드:
- 각 Step 의 **Pass 비율 변화** (%)
- **만성 flaky** (3회 이상 연속 flaky)
- **반복 회귀** (지난 5 run 중 2회 이상 fail→pass→fail 왕복)
- **한 번도 pass 없는 Step** (신호: QA 작성 자체가 잘못됐거나 기능 미구현)

### 7.3 공통 원인 클러스터링

> **[Opus 4.7 / 1M 활용]** 서브 분할 없이 메인에서 직접 **다중 run 동시 로드** 후 교차:
...
실패·flaky 증거 교차 분석:
- **콘솔 에러 메시지 클러스터링**: 동일 에러 문자열이 N개 Step → 공통 원인 1건
- **네트워크 URL 클러스터링**: 같은 엔드포인트 다수 실패 → 백엔드 공통 원인
- **DOM selector 클러스터링**: 같은 selector 다수 stale → 프론트 공통 원인
- **소요 시간 이상치**: P90 > P50 × 3 → 외부 의존성 지연

클러스터 결과는 §8.4 반복 실패 패턴의 근거로 재사용.
...
**Deep 강화**: QA 매트릭스뿐만 아니라 **실제 코드** 를 Grep 해서 QA 에 빠진 분기 탐지. 예: 코드에 `if (userType === 'ADMIN')` 인데 QA 에 ADMIN 케이스 없음 → 누락.

### 8.2 숨은 의존성

**Deep 강화**: 네트워크·DOM 교차 분석 + **코드에서 실제 의존성 그래프** 추출 (Grep `import` / `@Autowired` / `@Inject`).
...
**Deep 강화**: §7.3 공통 원인 클러스터링 결과 + 과거 run 매칭 → **만성 패턴** 식별.
...
§8.1~§8.6 + §8.8~§8.10 의 액션을 **P1/P2/P3 재분류** + 중복 해소 + 의존 순서 명시.

### 8.8 [Deep 전용] 코드 ↔ QA 일관성 구멍

**트리거**: §3 에서 수집한 코드와 QA 가 어긋남.

**탐지 방법**:
- 실패 Step 의 증거가 가리키는 코드 경로 Read
- QA 의 기대값 ↔ 실제 코드 로직 대조
- SRS 규격 ↔ 구현 대조
...
- 원본 P1/P2/P3 ↔ 재산정 우선순위 비교

**출력 예**:
```
- **[추정][P1 재산정]** Step 5.1 i18n 검색은 원본 P2 였지만, {고객사} 시연이 이번 주 → P1 승격
...
**출력 예**:
```
- **[추정][P2]** QA 가 2주 전 작성. 그 사이 TODO-08 이 "X 동작" 에서 "Y 동작" 으로 변경 → QA Step 2.4 기대값 stale
...
(`/qa-run-light` §8.8 과 동일)

**Deep 강화**:
- 과거 run 의 개입 기록도 조인 → **반복 개입 패턴** 식별
- 같은 유형 3회 이상 누적 → §8.7 다음 릴리즈 개선 액션에 **P1 자동화 액션** 강제 편입
...
**규칙**: 모든 P1 액션 + 상위 3 P2 액션. 나머지는 §8 에 공수만.
...
> **[Opus 4.7 / 메타인지]** 핵심 결론 Top 3 (P1 액션·코드↔QA 일관성 구멍·시맨틱 우선순위 재산정)에 대해:
...
- [ ] §8.8~§8.10 Deep 전용 3카테고리가 적절히 채워져 있는가
- [ ] 한눈에 (배너) 가 30초 내 상태 파악 가능
- [ ] 사용자 개입 1건 이상이면 §8.11 회고 + "자동화 가능성 + 다음 run 개선안"
...
**사용자 개입**: {interventions}건 (자동화 가능 {A} / 수동 유지 {M}) — §8.11 회고
...
| 로그인·CAPTCHA·세션만료·블로킹모달 | §5.4 막힘 감지 | 즉시 AskUserQuestion + §8.11 회고 |
| 같은 막힘 3회 이상 | 카운터 per 유형 | 일괄 처리 모드 |
| QA 포맷 위반 | Step 매치 < 80% | AskUserQuestion |
| Deep 맥락 과다 | Read 줄 수 > 600K | AskUserQuestion (범위 축소) |
| 세션 끊김 | 컨텍스트 한계 | 부분 리포트, `[PARTIAL]` |
| 스크린샷 용량 폭발 | du > 20MB | AskUserQuestion |
| 만성 flaky 3회 연속 | 과거 run 조인 | `chronic-flaky` → §8.7 P2 강제 |
...
| recommended-runner: light 인데 deep 호출 | §1.1 | AskUserQuestion (Deep 강행 / Light 전환 / 중단) |
...
- **측정 vs 추론 구분** — `[확신]` 인용, `[추정]`·`[가설]` 해석
- **인사이트 비어 있을 수 있음** — §8.12 로 정직하게
- **Deep 비용 인식** — 매 실행 1M 사용. Light 가 기본인 이유
- **AI 시각 판정 한계** — Opus 4.7 multimodal 강력하지만 CSS 픽셀 단위 정합 어려움. `[시각판정]` 태그 명시
- **dry-run 권장** — 처음 QA 는 `--dry-run` 으로 맥락·Step 확인
- **권한 모드** — MCP 호출 연쇄 → 사전 자동 허용 권장
...
**[환경·규모]** 1M 맥락 로드 대상 600K줄 이상
- 신호: 대상 코드베이스·문서 합산 라인 수 600,000+
