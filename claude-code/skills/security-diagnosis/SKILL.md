...
description: 보안과 컴플라이언스 관점 정기 진단. 자격 정보 관리, 인증과 인가, 입력 검증, 의존성 CVE, 로깅과 모니터링, 정책 준수를 종합. /security-review (PR 단위) 의 정기 버전으로, 코드베이스 전체 보안 자세를 진단.
...
**공격면을 본다.** `/security-review` 가 PR 단위라면, 이 스킬은 **코드베이스 전체의 보안 자세를 정기 진단**한다. 분기 또는 반기 단위 호출.

역할:
1. **자격 정보와 비밀 관리**: 평문 시크릿, 환경 변수 누설, 시크릿 회전 정책
2. **인증과 인가**: 인증 메커니즘, 세션 관리, RBAC, 권한 우회 가능성
3. **입력 검증과 주입**: XSS, SQL/NoSQL injection, CSRF, SSRF, 파일 업로드
4. **의존성과 CVE**: 알려진 취약점, 미사용 의존성, 라이선스 리스크
5. **로깅과 모니터링**: 민감 정보 로그 노출, 감사 로그 무결성, 알림 체계
6. **컴플라이언스와 정책**: 개인정보 처리, 데이터 보존, 사내 보안 정책 준수
...
3. **공개 보고 금지.** 보고서에 구체 exploit 페이로드 포함 금지. 패턴과 위치만
4. **민감 정보 마스킹.** 발견된 시크릿과 키는 마스킹하여 보고서에 기록 (보고서 자체가 유출원이 안 되게)
...
| 소스 | 신뢰도 | 보안 관점에서의 가치 |
|------|:------:|-----------------|
| 코드 (Grep) | 5/5 | 객관적 사실 (위치와 패턴) |
| 의존성 파일 | 5/5 | CVE 매칭 가능 |
| .env / config | 4/5 | 시크릿 관리 상태 |
| biz-rules.md | 4/5 | 권한과 접근 정책 명시된 곳 |
| WebSearch CVE DB | 3/5 | 공개 취약점 정보 (실제 노출은 별도 검증) |
| auto memory | 3/5 | 사용자가 교정한 보안 사실 |
| 인시던트 이력 (issues/) | 4/5 | 과거 실제 사건 |
...
| 환경 설정 예시 | `.env.example`, `application-*.properties` | 선택 | 자격정보 하드코딩 점검을 파일 grep 만으로 수행 (신뢰도 2/5) |
| 의존성 현황 | 빌드 파일 + `package-lock.json` / `pnpm-lock.yaml` 등 | 선택 | 취약 의존 목록 분석 생략, CVE 검토 수동 |

## 프로젝트 컨텍스트 (필수, 호출 전 read)

다음 파일이 존재하면 우선 read:
- `CLAUDE.md` (자동 로드, 보안 코딩과 금지사항)
- `bot/INDEX.md` 또는 `.local.claude/INDEX.md`
- `.local.claude/biz-rules.md` (권한과 인증 규칙)
...
| `auth` | **인증과 인가**: 파트 2만 | ~15분 |
| `injection` | **주입과 입력 검증**: 파트 3만 | ~15분 |
| `deps` | **의존성과 CVE**: 파트 4만 | ~10분 |
...
2. **전제 검증**: 이 발견이 "악용 가능한 취약점" 이려면 어떤 전제가 필요한가? (예: 평문 시크릿의 경우 git 히스토리 노출 or 실제 접근 가능 경로)
3. **반대 증거 탐색**: "이미 보호 레이어가 있는가?" / "false positive 가능성?" 같은 반박 1개 이상 탐색. 위협 모델의 자산과 공격자 가정과 정합성 재확인

반박 유효 시 P1에서 P2로 강등 또는 본문 수정, 부분 반박 시 "단, {전제}" 인라인 추가.

```markdown
## 자기 검증

1. **위협 모델 정합성**: 위협 모델의 자산과 공격자가 모든 PART 에 반영
2. **악용 가능성 평가**: 단순 패턴 매칭이 아닌 실제 노출 경로 확인
3. **민감 정보 마스킹**: 발견된 시크릿과 키 모두 마스킹
...
- P1 발견 시 `/review` 로 즉시 PR 단위 보안 리뷰 진행 (보안 중심 관점으로 수행)
- 의존성 패치 다수면 별도 패치 PR
- 컴플라이언스 갭은 `/business-diagnosis` 와 연계 (예산과 리스크)
- 인증과 인가 재설계 필요 시 `/srs` 로 요구사항 정의
...
- **공개 exploit 페이로드 금지.** 패턴과 위치만 기록
- **민감 정보 마스킹 필수.** 발견된 시크릿 값 자체를 보고서에 쓰지 않음
- **악용 가능성 vs 이론 구분.** 단순 CVSS 점수보다 실제 노출 경로 우선
- **위협 모델 합의 후 분석.** 자산과 공격자 가정 없으면 모든 발견이 중요해 보여 우선순위 무의미
- **자동 도구 결과 보조.** 수동 점검은 한계가 있으므로 npm audit / OWASP DC 등 결과를 사용자가 제공하면 통합
...
- 대응: 설치 가이드 안내 후 사용자에게 기존 결과 파일 경로 질문 후 수동 입력으로 진행

**[데이터 결함]** CVE DB WebSearch 실패 또는 응답 부재
- 신호: CVE 조회 중 네트워크 오류 또는 결과 0건
- 대응: `[CVE 미검증]` 태그 병기 + 오프라인 룰(의존성 버전과 관용적 위험) 기반 간이 판정
