...
| 디자인 원칙 (0절 체크리스트) | `assets/design-principles.md` | **필수** | 품질 기준 SSoT 부재로 중단 |
| 컴포넌트 + kind 프리셋 | `assets/components.md` | **필수** | 본문 채움 불가로 중단 |
| 내용 추출 10원칙 | `assets/content-rules.md` | **필수** | 핵심 추출 규칙 부재로 중단 |
...
- 인자가 없으면(또는 topic 단어만) 현재 세션 대화 전체 흡수: 한 일 / 정한 일 / 다음.
...
5. `template.html`의 `{{TITLE}}{{SUBTITLE}}{{ICON}}{{TOC_ITEMS}}{{MAIN}}` 를 components.md 컴포넌트로 채움. 적응형: 내용 구조가 배치를 결정(고정 틀 강제 금지). 헤더 `{{ICON}}`는 kind에 맞는 헤더 아이콘(components.md "헤더 아이콘")으로 채움. 빈 장식이 아니라 종류 표시이며, 매칭 없으면 default.
   - **소스 문서 모드**: `.seg`(원문 토글) 유지 + 핵심 블록을 `.only-easy`(쉬운 설명) / `.only-raw`(원문 verbatim)로 병기해 해석 검증성 보존.
...
10. 사용자 피드백 시 `assets/playbook.md` 해당 kind에 1줄 append(날짜 기입, 간결하게).

## 비협상 제약

- 원본 절대 불변. 항상 별도 `.html`.
- 스킨은 `template.html` 그대로(라이트). 다크 금지. 이모지 금지.
- 외부 의존 추가 금지(Pretendard CDN 1개만). 그 외 CSS와 JS는 인라인.
- affordance 정직: 동작 요소는 `button`/`a`, 정적은 `.tag`(가짜 버튼 금지).
- 고정 template N개 금지. 적응형 배치 + playbook 누적.
- 디자인과 문서 품질 기준은 `design-principles.md`가 SSoT이므로 여기 재기술 금지.
...
| 수신자에 맞는 메시지/보고로 가공해 전달 | /draft | 다루지 않음, 전달 메시지는 위임 |
| 디렉터리 문서를 AI용 bot/ + 사람용 human/ 으로 재구성 | /dualize-docs | 다루지 않음, 디렉터리 재구성은 위임 |

## 검증 시나리오

공통 3블록(빈 / 부분 / 풀 데이터)은 **CONTRACT 6-1절** 참조. 단, `assets/*` 필수 자산이 빠지면 "빈 프로젝트" 가 아니라 스킬 설치 손상으로 간주하여 즉시 중단한다.
