...
| 질문 | 담당 | 이 스킬에서 |
|------|------|-----------|
| "신규 프로젝트 첫 온보딩 분석과 6산출물 생성" | /on-boarding | 다루지 않음 (SETUP.md 최초 생성은 on-boarding) |
| "SETUP.md 환경 점검, 갱신, 암묵지식, 트러블슈팅 누적" | **이 스킬** | ✓ 핵심 |
| "트러블슈팅이 도메인 규칙이나 코드 버그로 판명" | /learn 또는 /cs | 다루지 않음 (검증과 이슈 추적은 해당 스킬) |
...
| MCP/Hook 설정 | `.mcp.json`, `.claude/settings.json` | 선택 | MCP와 Hook 섹션 생략 |
...
| `/setup-guide update` | **갱신** | 코드/설정 변경을 감지해 SETUP.md 에 자동 반영 |
| `/setup-guide capture <내용>` | **수확** | 사용자 입력으로 암묵 지식과 트러블슈팅 추가 |
...
1. SETUP.md 의 2절(필수 설치) 표를 Read 하여 각 항목의 **확인 명령** 추출
...
- `.mcp.json` 신규 생성됨. SETUP.md 5절 갱신 권장
```
...
2. SETUP.md 2절(필수 설치), 3절(환경 변수), 4절(IDE), 5절(Claude Code), 6절(첫 실행) 의 현재 내용 추출
3. **diff 계산**:
   - 추가됨: 자동 추출 소스에 있으나 SETUP.md 에 없는 것
   - 변경됨: 양쪽 다 있으나 값이 다른 것 (예: Node 버전 18에서 20으로)
...
4. **반영 전 사용자 확인** (최대 5건씩 묶어서 한 번에):
   ...
   변경 (1):
   - Node.js: 18.x 에서 20.11.x 로 (.nvmrc 변경)
...
**주의**: 1절, 7절, 8절 (사전 요구사항, 암묵 지식, 트러블슈팅) 은 자동 갱신 대상이 아님. 손대지 않음.

### 3. 수확 모드 (`capture`)

**목적**: 사용자가 셋업 중 발견한 암묵 지식과 트러블슈팅을 SETUP.md 에 누적.

`$ARGUMENTS` 가 자유 입력. 분류 자동 판단:

| 입력 패턴 | 분류 | 반영 위치 |
|----------|------|---------|
| "VPN 어떻게 받지", "DB 비밀번호", "Slack 채널" | 암묵 지식 | 7절 사람에게 들어야 할 것 |
| "에러 났는데 ~로 해결", "이 명령 안 되면 ~" | 트러블슈팅 | 8절 트러블슈팅 |
| "이 명령 추가해야 함", "Make target 이 ~" | 첫 실행 보강 | 6절 첫 실행 |
| "OS 별 차이", "M1 Mac 에서는 ~" | 사전 요구사항 | 1절 사전 요구사항 |
...
- SETUP.md 는 사람이 직접 손대는 문서. 자동 갱신 시 사용자 추가 내용 (7절, 8절) 보존 필수
...
| >500줄 | **분리 강제**. 8절 트러블슈팅을 `human/SETUP-TROUBLESHOOTING.md` 로 이전 |

**분리 기준**:
- `human/SETUP.md` (메인): 1절~7절, 9절 (셋업 절차 본체)
- `human/SETUP-TROUBLESHOOTING.md` (수동 참조): 8절 트러블슈팅만 (가장 빠르게 누적되는 영역)
- 메인 파일에 `트러블슈팅: [SETUP-TROUBLESHOOTING.md](./SETUP-TROUBLESHOOTING.md)` 링크 안내

추가로 8절 트러블슈팅이 200줄 넘으면 OS별, 역할별 sub-section 으로 재구조화 제안 (Mac/Windows/Linux, FE/BE 등).

## 다른 스킬 연결

- SETUP.md 가 없으면 `/on-boarding` 안내
- 트러블슈팅이 도메인 규칙이나 코드 버그로 의심되면 `/learn` 또는 `/cs` 안내
- 환경 변경이 빈번하면 (월 5회+) 사용자에게 hook 자동화 (`PostToolUse` 또는 `SessionStart`) 제안

## 제약조건

- **SETUP.md 만 수정**. 자동 추출 소스 파일 (`.nvmrc`, `package.json` 등) 은 절대 수정하지 않음
- **7절 (암묵 지식), 8절 (트러블슈팅) 은 자동 갱신 안 함**. 사용자 명시 입력 (`capture` 모드) 으로만 변경
- **자동 갱신 전 사용자 확인 필수**. "전체 반영 / 선택 반영 / 취소" 선택지 제시
- **트러블슈팅은 삭제 금지**. 누적만. 해결된 문제도 다음 사람에게 가치 있음
- **점검 모드의 Bash 실행은 read-only 명령만**. 설치나 수정 명령은 절대 실행하지 않음 (사용자에게 안내만)
...
- 신호: 런타임과 버전 명시 파일 모두 부재
- 대응: 사용자에게 런타임 이름과 버전 수동 입력 요청 + `[수동 입력]` 태그로 출처 표시
