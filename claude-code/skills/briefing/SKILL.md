...
| `.local.claude/biz-rules.md` (있을 시) | 도메인 규칙·상태 전이 — 변경의 도메인 영향 해석에 필요 |
| `.local.claude/modules/*.md` (있을 시) | 변경된 모듈의 서비스·테이블·이벤트 연동 |
| `bot/*.md` 또는 `.local.claude/ONBOARDING.md` (있을 시) | 모듈 간 흐름·이벤트 |
| 팀원 매핑 파일 (있을 시) | git author → 팀원 닉네임 매핑 |
...
`.local.claude/team.md` (조직 구조 + 닉네임-실명 매핑) 가 있으면 read 후 git author → 팀원 닉네임 매핑.

### 4. 도메인 맥락 (위 컨텍스트 파일에서 read)
- `biz-rules.md` — 상태 전이, 비즈니스 규칙 위반 여부 점검
- `modules/{name}.md` — 변경된 모듈의 서비스·이벤트 연동
...
5. **공통 모듈** 변경은 전체 영향 — 반드시 주의사항에 기재. 공통 모듈 식별은 `CLAUDE.md` 의 구조 섹션 또는 `bot/INDEX.md` 참조.
...
**영향도 기준**:
- **높음** — 기존 업무 흐름·데이터 처리 변경, 사용자 안내 필요 가능
...
- 팀원 매핑 파일 read 후 author → 닉네임 매핑
- `.local.claude/people/{닉네임}.md` 가 있으면 Read — 담당 모듈/강점 참고
...
**모듈 → 문서 파일명 매핑**: 프로젝트의 패키지 구조에서 자동 추론. 명시 매핑이 필요하면 `.local.claude/modules/_INDEX.md` 같은 파일에 정의 (있을 시 read).

### 처리 흐름

```
변경된 모듈 식별 (브리핑 분석에서 이미 파악됨)
  → 모듈명 → 문서 파일명 매핑 (자동 또는 INDEX 참조)
  → .local.claude/modules/{name}.md 존재 확인
    → 문서 없으면: 스킵
    → 문서 있으면: 감지 기준 패턴 매칭
      → 매칭되면:
        1. 기존 문서 Read
...
`biz-rules.md` 의 상태 코드·도메인 키워드를 read 한 뒤, diff 에서 다음 패턴을 감지:

| 감지 대상 | diff 패턴 |
|----------|----------|
| 상태 코드 변경 | biz-rules.md 의 상태 코드·`*Status` setter 에 새 문자열 리터럴 |
...
Frontmatter (CONTRACT §7-2 표준): `category: briefing, retention: 30d, harvest_targets: [biz-rules.md, modules/*.md]`
...
- **프로젝트 컨텍스트 파일이 없으면 일반 git diff 분석만 수행** — 도메인 해석 강제하지 않음
