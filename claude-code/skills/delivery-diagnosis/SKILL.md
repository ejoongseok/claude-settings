...
description: 전달·실행 관점 진단 — 일정 예측 가능성, WIP·차단 요인, 작업 흐름, 재작업률, 자원·역량 매칭에 집중. 사업·제품·기술 진단이 "무엇을·왜" 라면 이 스킬은 "어떻게 흘러가고 있는가" 에 답한다.
...
3. **차단·블로커 분석** — 누가 무엇에 차단되고 있는가, 인접 팀 의존성
4. **품질 vs 속도 트레이드오프** — 재작업률, 핫픽스 비율, 기술 부채 누적
5. **자원·역량 매칭** — 일과 사람의 매칭 (중복·갭·번아웃 신호)
...
2. **데이터 우선, 일화는 보조.** "느낌상 느려진다" 보다 "lead time p50 이 X→Y" 가 우선
3. **추정값은 명시.** 정확한 데이터 없으면 `(근사 추정, ★★)` 표기
4. **선행 신호 vs 후행 신호.** 일정 미준수는 후행. WIP 증가·재작업 증가가 선행
5. **다른 진단 보고서 참조 시** 날짜·신뢰도 명시

### 데이터 소스 신뢰도

| 소스 | 신뢰도 | 전달 관점에서의 가치 |
|------|:------:|-----------------|
| issues/ (이슈 트래커) | ★★★★★ | 약속·완료·기한의 객관적 기록 |
| git log / PR | ★★★★★ | 실제 산출 시점, 리뷰 사이클 |
| daily-todos/ | ★★★★ | 일일 작업 흐름, 차단 신호 |
| projects/*/ | ★★★★ | 프로젝트별 진행 상태 |
| daily/ | ★★★ | 회고 (재작업·차단 정성 신호) |
| briefing/ | ★★★ | 주간 변경 흐름 |
| meetings/ | ★★★ | 의사결정·차단 해소 맥락 |
| memo/ | ★★ | 비공식 관찰 (비난 금지 원칙 적용) |
| auto memory | ★★★★ | 교정된 사실 |
...
다음 파일이 존재하면 우선 read:
- `CLAUDE.md` (자동 로드, 프로젝트 구조·컨벤션)
- `bot/INDEX.md` 또는 `.local.claude/INDEX.md`
- `.local.claude/team.md` (팀 구성·역량)
...
**[기간 내]**
- `.local.claude/daily/` 최근 N건 — 차단·재작업 정성 신호
...
| 메트릭 | 산식 | 데이터 소스 |
|--------|-----|----------|
| Lead time (p50, p90) | PR createdAt → mergedAt | gh pr list |
| Cycle time | 첫 커밋 → merge | git log + PR |
...
| 재작업률 | revert · 핫픽스 PR / 전체 PR | PR title/label Grep |
| 추정 갭 | issue label 의 estimate vs 실제 종료 | gh issue list |

데이터 부족 시 `(근사 추정, ★★)` 표기. 측정 불가 항목은 `[측정 불가]` 명시.
...
WIP 한계 초과 인원 → 차단·재작업·번아웃 위험 시그널.

**→ 의사결정 포인트:** 워크플로우에서 가장 급한 정체 구간 1줄

---

## PART 3: 차단·블로커 분석 (전달 관점에서만 본다)
...
같은 유형 차단이 N회 이상 반복 → **시스템 문제**. 개선 후보:
- 권한 사전 부여 / 접근 자동화
- 의사결정 위임 명확화
- 외부 응답 SLA 합의
...
재작업률 > 20% → 품질 손실. 0% 에 가까움 → 과도한 신중·테스트 부족 가능성.
...
team.md 의 멤버 역량 + 진행 작업 매칭. 가능 멤버 1명뿐인 영역 → bus factor 위험.
...
2. **전제 검증**: 권고가 유효하려면 어떤 전제가 필요한가? (예: "WIP 축소" → 작업 단위 분해 가능 / 외부 의존 차단 해소 가능)
3. **반대 증거 탐색**: "왜 이미 하지 않았나?" / "현재가 충분하다면?" — 반박 1개 이상 탐색. 사람에 대한 추측이 시스템 분석으로 바뀌었는지 재점검
...
2. **인물 비난**: 0건 (사람 X, 시스템·패턴 O)
3. **추정 표기**: 추정값에 모두 ★ 신뢰도 부여
...
- **데이터 우선, 일화는 보조.** 정량 메트릭 없으면 `(근사 추정, ★★)` 표기
...
- 대응: "선행 신호 파악 제한" 안내 + 지연·WIP 추세 분석 생략, 스냅샷 중심 진단

**[도메인 특수성]** 단일 배포 팀(역할 분리 없음)
- 신호: 팀 1명~소수 / 핸드오프 경계 부재
- 대응: WIP 한계·핸드오프 지연 분석 생략, 리드타임·배포 빈도만 보고
