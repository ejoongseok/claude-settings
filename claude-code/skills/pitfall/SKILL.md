---
name: pitfall
description: "현재 세션의 시행착오를 시도, 실패, 원인, 해결, 재방문 신호의 5요소로 캡처합니다. 보편 교훈은 메모리에, 프로젝트 특수는 문서에 저장합니다. 동일 함정 재방문 차단을 위해 시행착오 인지 후 어느 시점에든 호출하세요(시점 무관)."
...
- 매체 판단: 보편 교훈은 메모리, 프로젝트 특수는 문서, 양쪽 해당이면 둘 다
...
| 해결(Fix) | 어떻게 해결하거나 회피했는가 | "V 함수로 우회" |
...
조기 종료: 현재 세션에 시행착오 식별 불가면 "추출할 시행착오가 없습니다" 안내 후 종료.
...
- 문서:
  - `.local.claude/learn/` 존재 시 `.local.claude/learn/{YYYY-MM-DD}-pitfall.md`
  - 모듈 특화면 `.local.claude/modules/{name}.md` 의 "함정" 섹션 추가
  - 그 외는 `docs/pitfalls.md` (부재 시 신규)

동일 시행착오 사전 검사:

1. Grep으로 재방문 신호와 키워드 검색 ("저장 위치 결정"의 sink 후보 전체)
2. 발견 시 SKIP + 위치 안내 ("이미 기록됨: {파일}:{줄}"), 신규 저장 안 함

AskUserQuestion으로 매체와 위치 승인 후 Phase 3 진입.
...
1. 메모리 저장 (해당 시):
   - type 결정: 보편이면 feedback, 프로젝트 특수면 project
   - auto memory 시스템 frontmatter (`name`, `description`, `type`) + 5요소 본문
   - `MEMORY.md` 인덱스에 한 줄 추가: `- [{title}]({file}.md): {재방문 신호}`
2. 문서 저장 (해당 시):
   - 기존 파일이면 Edit (5요소 표 추가)
   - 신규 파일이면 Write (CONTRACT 7-2절 frontmatter + 5요소 표 + 첫 줄 `> /pitfall 으로 생성 (YYYY-MM-DD)`)
3. 보고:
...
- 문서 파일: CONTRACT 7-2절 frontmatter + 5요소 표 + 첫 줄 `> /pitfall 으로 생성 (YYYY-MM-DD)`
...
| `MEMORY.md` (자동 로드) | 200 | 없음 | `/memory-garden` |
| `docs/pitfalls.md` | 300 | 500 | 카테고리나 시기별 분리 |
...
| 메모리 인덱스 정리와 아카이브 | `/memory-garden` | [다루지 않음] |
...
- 시행착오 3건+ 발견 시 `/session-glean`
- `MEMORY.md` 200줄 근접 시 `/memory-garden`
- 코드 레벨 검증 필요 시 `/learn`
- 데이터 부재 (CLAUDE.md와 메모리 모두 없음) 시 `/on-boarding`
