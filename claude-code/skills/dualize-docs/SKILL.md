...
- **셸 명령은 POSIX 호환 가정** — `find`/`wc`/`grep`/`mv` 등 표준 유틸 사용, 경로 구분자 `/`. 실행 환경의 셸·유틸 차이는 `CLAUDE.md` 또는 프로젝트 설정을 따른다 (특정 OS 가정 박지 않음)
...
> **[메타인지]** Pass 3 insights.md 작성 완료 직후 Adversarial Review — 핵심 발견(모순·숨어있는 것·맹점·향후 방향 P1) Top 3 각각에 대해:
...
- **재생성·아카이브는 사용자 승인 후** — `rm -rf`·`mv` 전 AskUserQuestion 확인 (3·9단계). 무단 덮어쓰기 금지.
...
- **사실은 bot/ 단일 소스** — 같은 사실을 bot·human 양쪽에 중복 기재 금지. 코드 레퍼런스는 완전 경로(`...` 축약 금지).
