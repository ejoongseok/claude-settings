...
| 비즈니스 규칙 | `.local.claude/biz-rules.md` | 선택 | 8.5절 도메인 규칙 구멍 카테고리를 "해당 없음" 한 줄로 대체 |
...
- `light` 면 그대로 진행
- `deep` 이면 AskUserQuestion: "이 QA 는 deep 권장 ({recommended-runner-reason}). 그래도 Light 로 진행?"
...
분류:
- **assertion-eval**: `>= 3`, `=== X` 등은 `browser_evaluate` 결과와 비교
- **network-contract**: `HTTP 200`, `POST /api/...` 는 `browser_network_requests` 필터 검증
- **natural-language**: 자연어 기대는 **Light 모드에선 `manual-pending`** (사용자 확인 섹션)

자연어 기대값이 5건 이상이면 1.1절 추천 모드 확인과 별도로 한 번 더 안내: "자연어 기대값 N건. Deep 의 AI 시각 판정 권장".
...
직후 표의 각 행마다 Step 반복 실행. Step 본문 `{변수}` 와 `{케이스}` 치환. 결과 ID 에 `-row1`, `-row2` suffix.

### 2.5 수동 Step / 우선순위 / 도메인

- `\(MCP\s+밖,\s*수동\)` 은 `skip-manual` 태그
- `[URGENT]P1` 은 P1, `[참고]P2` 는 P2, `[정상]P3` 은 P3, 미지정은 P2 기본 (경고 출력)
- Frontmatter `domain: standalone` vs `domain-native` 는 8.5절 도메인 규칙 구멍 카테고리 분기에 사용

### 2.6 파서 매치율

Step 헤더 매치율 < 80% 이면 AskUserQuestion (재생성 권장 / 강행 / 중단).
...
`## 0. 선수 조건` 체크박스 순회:
1. 파일 경로는 `test -f` / `ls`
2. HTTP URL 은 `browser_navigate` + `browser_console_messages(level=error)` 로 404/CORS 탐지
3. 프로세스 기동 여부는 자동 검증 불가하므로 사용자 확인 요청 리스트로

실패 ≥ 1 건이면 AskUserQuestion (강제 진행 / 중단 / 수동 Step 만 출력).
...
`--dry-run` 은 여기서 종료 (Step 목록과 예상 소요 출력).
...
2. MCP 시퀀스 순차 호출 (navigate, snapshot, click, wait_for, evaluate 순 등)
3. assertion 비교:
   - `assertion-eval`: 결과와 기대값 비교
...
assertion 실패 또는 MCP 예외 시 **1회 자동 재시도** (단, 5.4절 막힘 감지 신호 시 retry 즉시 중단):
- 2회 모두 실패하면 `fail`
- 1실패 + 1성공이면 `flaky`
- 2회 모두 성공하면 `pass` (재시도 발생은 경고 기록)

### 5.3 stop-on-fail + 실행 간 격리

- `--stop-on-fail` + P1 fail 이면 즉시 중단, 나머지 `not-run`
...
**대응 플로우**:
1. **Retry 즉시 중단** (5.2절 Retry 규칙보다 우선)
...
4. "수동 완료" 재실행:
   - 성공하면 `pass-after-manual`
   - 실패하면 `fail-after-manual` (추가 retry 없음)
5. **개입 기록** 수집 (8.8절 회고 입력): Step ID, 감지 신호, 타임스탬프

**반복 감지 제한**: 같은 유형 3회 이상이면 일괄 처리 모드 ("일괄 수동완료 / 일괄 skip / 전체 중단")
...
비교 결과:
- 새 회귀 (이전 pass 가 이번 fail 로): N건
- fix (이전 fail 이 이번 pass 로): N건
...
> 2. 전제 검증 (6필드가 누락 없이 채워지는가, 특히 신뢰도와 우선순위가 증거로 환원되는가)
...
**트리거**: 외부 API 응답 지연, 소요 분산 큼, 다국어와 모바일 미검증.

### 8.7 다음 릴리즈 개선 액션

8.1~8.6절의 액션을 **P1/P2/P3 재분류** + 중복 해소 + 의존 순서 명시. "P1 2건 이상이면 블로킹 순서 명시".
...
| 개입 유형 | 자동화 가능성 | 권장 조치 |
|---------|-------------|----------|
| 로그인 | 높음 | QA 의 '0. 선수 조건' 절에 "세션 미리 확보" + seed 명시 |
...
| 파일 업로드 경로 부재 | 중간 | QA 의 '0. 선수 조건' 절에 fixture 경로 명시 |
...
**사용자 개입**: {interventions}건. 리포트 6.8절 회고 참조
...
| 로그인, CAPTCHA, 세션만료, 블로킹모달 | 5.4절 막힘 감지 | **즉시** AskUserQuestion (재시도 금지) + 8.8절 회고 입력 |
...
| recommended-runner: deep 인데 light 호출 | 1.1절 추천 모드 확인 | AskUserQuestion (Light 강행 / Deep 전환 / 중단) |
...
- **원본 QA-*.md 불변**: 읽기 전용
- **서브에이전트 위임 금지**
- **측정 vs 추론 구분**: `[확신]` 은 인용, `[추정]` 과 `[가설]` 은 해석
- **인사이트 비어 있을 수 있음**: 8.9절 인사이트 부재 처리로 정직하게 표기
...
**[사용자 개입 필요]** 로그인과 CAPTCHA 감지
- 신호: Playwright 스냅샷에 로그인 폼, OTP, CAPTCHA 요소 또는 302 인증 리다이렉트
- 대응: 실행을 중단하고 사용자에게 직접 로그인 요청, 재개 후 해당 스텝 재시도 (최대 2회)
