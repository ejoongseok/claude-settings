...
| "변환된 문서에서 지식 추출과 지식 베이스 반영" | /absorb | 다루지 않음 (parse-doc 산출물을 absorb 입력으로 사용) |
| "parsed/ 누적 문서 흡수 확인, 아카이브, 만료 관리" | /garden | 다루지 않음 (수명 관리는 garden) |
| "사람용/AI용 이중 문서 분리와 재구성" | /dualize-docs | 다루지 않음 (이 스킬은 원본을 Markdown 으로 변환하는 것까지만) |
...
| `.xls` | [OK] 지원 | xlrd 로 Markdown 테이블 변환 |
...
| 대용량 PDF (100MB+) | 타임아웃 | 페이지 분할: `npx kordoc 파일.pdf --pages 1-20` | 없음 |
| PPTX 차트/SmartArt | 해당 요소만 텍스트 미추출 | 없음 | 차트는 이미지로 추출되므로 Claude 멀티모달 해석으로 대체 |
| HWP/HWPX 이미지 | kordoc이 이미지 미추출 | HWP를 PDF로 변환 후 pdf_images.py로 추출 | 한컴오피스에서 PDF로 내보낸 뒤 kordoc(텍스트) + pdf_images.py(이미지) |
...
> **[이미지 해석]** 시스템 아키텍처 다이어그램. 클라이언트에서 API Gateway 를 거쳐 마이크로서비스 3개(인증, 주문, 재고), 그 뒤 PostgreSQL DB 로 이어지는 구조.
```
...
parsed/ 산출물은 즉시 자동 로드 대상이 아니므로(`@` 미사용) 임계는 가독성과 후속 흡수 효율 기준. 대용량 PDF(100MB+ / 200페이지+)는 `pages` 분할로 변환 자체를 나눈다 (검증 시나리오 참조). 누적 정리와 아카이브는 `/garden` 위임.

## 제약조건

- 원본 파일은 반드시 `.local.claude/docs/original/`에 위치해야 함
- 파싱 결과는 Markdown (텍스트 + 테이블)
- PPTX 이미지: pptx2md.py가 자동 추출한 뒤 Claude 멀티모달로 해석해 텍스트 설명 삽입
- PDF 이미지: pdf_images.py(PyMuPDF)로 추출 후 동일하게 해석
