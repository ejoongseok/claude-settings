...
1. **재무는 역산 추정.** 공개 정보 없으므로 항상 `(역산 추정, 신뢰도 2/5)` + 범위(보수적~낙관적)
2. **대표 의도는 간접 추론.** 직접 인터뷰 없이는 `[간접 추론]` 태그 필수
3. **복수 출처 원칙.** 개인 평가는 2개+ 출처. 단일 출처면 `[단일 출처]`
4. **데이터 모순 시 양쪽 기술.** `[데이터 충돌]` 태그, 임의 선택 금지
5. **/tech-diagnosis, /product-diagnosis 보고서 참조 시** 해당 보고서 날짜와 신뢰도 명시
...
> - 로드: `team.md`, `customers/*.md` 전체, auto memory, /tech-diagnosis, /product-diagnosis 보고서, 이전 /business-diagnosis 보고서
...
- 고객별 ARPU 추정 `(역산 추정, 신뢰도 2/5)`
- 성장률: 보도 기반 + 기저 효과 분석

### 1-2. 비용 구조

- 인건비 비중 (N명 × 평균). 고정비의 대부분
- 인프라 비용 `(역산 추정, 신뢰도 2/5)`
...
4. **태그 점검**: `(역산 추정, 신뢰도 2/5)`, `[간접 추론]`, `[단일 출처]`, `[데이터 충돌]` 누락 없는지
5. **기술, 제품과의 정합성**: 이 보고서의 자원 배분이 /tech-diagnosis, /product-diagnosis 권고와 모순되지 않는지
...
- 기술 심층은 `/tech-diagnosis`
- 제품/고객 심층은 `/product-diagnosis`
- 투자자/이사회 보고는 `/draft`
- 채용 JD 는 `/prd`
- 기술 방향 발산은 `/brainstorm`
- 고객사 변동은 `/customer-profile`
- 긴급 리스크는 `/poc`

## 제약조건

- **사업 관점에 집중.** 기술 상세는 /tech-diagnosis 에, 제품/고객 상세는 /product-diagnosis 에 위임. 경영 임팩트만 본다
- **재무 인라인 표기.** `(역산 추정, 신뢰도 2/5)` 등장하는 모든 곳
