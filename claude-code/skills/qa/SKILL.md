...
| 이럴 때 | 어떤 스킬 |
|--------|---------|
| 회의, 요청, 아이디어를 비즈니스 맥락 문서로 | `/prd` |
| PRD 에서 기술 명세 + 경계와 예외 조건으로 | `/srs` |
| SRS 에서 Outside-In 개발 순서로 | `/todo` |
| **PRD/SRS/TODO 에서 QA 체크리스트로 (Happy, 경계, 예외, 부정, 변형, 보안, 동시성, i18n)** | **`/qa` (본 스킬)** |
| 배포 전 수동 체크리스트 | `/deploy-checklist` |
| 누적 문서를 AI/사람 두 독자층으로 재구성 + 메타 분석 | `/dualize-docs` |
...
- **수동 Step 허용**: 제목에 `(MCP 밖, 수동)` 표기 시 MCP 시퀀스와 실패 대응 요구 면제 (빌드, 촬영, OS 조작 등)
...
독립 프로젝트 판정 시 QA 산출물의 '2. 공통 검증' 절이 다음으로 대체:
- 콘솔 에러 0건
- 리소스 404 0건
- 한글 렌더 정상
- 빈 상태 안내
...
AskUserQuestion: "기존 QA-*.md 가 있습니다. 덮어쓸까요?" 로 묻고, 승인 시 진행.
...
서브에이전트 위임 금지. 1M context 로 입력 + 관련 문서 + 필요 시 관련 컨트롤러, Svc, DTO 코드까지 Grep 후 Read.
...
| # | 카테고리 | 의도 | 전형적 Step |
|---|---------|------|-----------|
| 1 | **Happy** | 정상 흐름이 끝까지 동작 | 중간값 입력 후 저장, 목록 반영 확인 |
...
| 5 | **Data Variation** | 입력 개수와 조합 다양성 | 0건, 1건, N건, 중복, 혼합 |
| 6 | **Security** | 권한과 주입 공격 방어 | 격리 우회, SQL 주입, XSS, CSRF |
| 7 | **Concurrency** | 동시성과 경쟁 상태 | 동시 저장, 락, 이벤트 재입력 |
| 8 | **i18n** | 다국어와 특수문자 | 한글, 영문, 이모지, 유니코드 |
...
| 다중 선택 후 일괄 처리 | 버튼 disabled | - | 여러 선택 | 최대 선택 수 | - | 같은 ID 2번 |
| 파일 업로드 | - | 단일 | 복수 | 최대 n개 | 용량 Max | 같은 파일명 |
| 검색 | "결과 없음" | - | - | 페이지 경계 | - | - |

### 4.5 상태 전이 변형 (Level 3~5)

- Happy: 허용 전이 순차 (예: 요청, 검토, 합의, 승인 순)
- Exception: **허용 안 된 전이 시도**: 서버 거부 확인 (예: 요청에서 승인으로 직행)
- Concurrency: 동시에 두 사용자가 같은 건을 전이하면 낙관적 락 충돌
...
| P2 | [참고]P2 | Boundary / Exception (정상 운영 중 재현 빈도 있음) / 상태 전이 (Happy 외) / 카디널리티 0과 N |
| P3 | [정상]P3 | i18n / 이모지 / 접근성 / Negative 중 극단값 (지수표기, 금지문자 등) / 카디널리티 Max와 중복 |
...
- [ ] `browser_evaluate`: `() => ...`
  - **기대**: ...
- [ ] `browser_network_requests` 로 POST /api/... 확인
...
제목에 `(MCP 밖, 수동)` 포함 시 스킬 자체 검증에서 MCP 시퀀스와 실패 대응 요구 면제. 수동 체크리스트만 허용:
```markdown
### Step Z: 빌드와 배포 (MCP 밖, 수동)
**수동 체크리스트**:
...
- P1 Step 3개 이상 (블로킹 다수라 Deep 의 예측 QA와 대안 비교 가치 큼)
- 자연어 기대값 다수 ("자연스럽게 정렬", "깨지지 않음" 등 5건 이상이면 AI 시각 판정 필요)
...
파일명:
- `SRS-{기능명}.md` 는 `QA-{기능명}.md` 로
- `TODO-{기능명}.md` 는 `QA-{기능명}.md` 로

---

> **[메타인지]** 최종 매트릭스 완성 직전, 핵심 결론 Top 3(P1 Step 선정, 필드 커버리지 판단, 도메인 분리 판정)에 대해:
> 1. 근거 재점검 (SRS의 경계조건과 예외흐름 인용 vs 생성 Step의 실제 매핑)
> 2. 전제 검증 (6필드, 즉 목적, 카테고리, 우선순위, 사전 상태, MCP 시퀀스, 실패 대응이 Step마다 모두 있는가)
...
- 독립 프로젝트 감지는 휴리스틱이므로 오분류 시 `--categories` 로 명시 수정
