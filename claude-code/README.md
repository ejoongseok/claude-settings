...
Claude-code-settings/
├── README.md            (이 파일)
├── CLAUDE(GLOBAL).md       전역 개인 행동 지침 드롭인 (분기형 판단 원리, 규칙 본문만)
├── CONTRACT.md             스킬 작성 단일 진실 원천 (13 항목)
├── calibration/         행동 지침 캘리브레이션 문제집 (월 1회 채점, 18문항)
├── hooks/               표현 린트 훅 (style_lint.py + 배선 안내)
├── rules/               참조 규칙 (표현 규칙, 외부 반출 문서 작성)
